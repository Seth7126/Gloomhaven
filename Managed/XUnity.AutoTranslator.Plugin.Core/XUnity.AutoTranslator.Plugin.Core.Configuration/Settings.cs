...
internal static class Settings
{
	...
	public static readonly float IncreaseBatchOperationsEvery = 30f;
	...
	public static float Timeout = 150f;
	...
	public static readonly float MaxTranslationsQueuedPerSecond = 5f;
	...
	public static void Configure()
	{
		try
		{
			TranslatorsPath = Path.Combine(new FileInfo(typeof(Settings).Assembly.Location).Directory.FullName, TranslatorsFolder);
			try
			{
				try
				{
			...
			{
				XuaLogger.AutoTranslator.Warn(e2, "An error occurred while trying to determine the game resolution.");
			}
			try
			{
				CachedProperty version = UnityTypes.TMP_Settings_Properties.Version;
				if (version != null)
				{
					TextMeshProVersion = (string)version.Get(null);
					XuaLogger.AutoTranslator.Info("Version of TextMesh Pro: " + TextMeshProVersion + ".");
				}
			}
			catch (Exception e3)
			{
				XuaLogger.AutoTranslator.Warn(e3, "An error occurred while trying to determine TextMesh Pro version.");
			}
			ServiceEndpoint = PluginEnvironment.Current.Preferences.GetOrDefault("Service", "Endpoint", "GoogleTranslateV2");
			...
			EnableIMGUI = PluginEnvironment.Current.Preferences.GetOrDefault("TextFrameworks", "EnableIMGUI", defaultValue: false);
			EnableUGUI = PluginEnvironment.Current.Preferences.GetOrDefault("TextFrameworks", "EnableUGUI", defaultValue: true);
			EnableNGUI = PluginEnvironment.Current.Preferences.GetOrDefault("TextFrameworks", "EnableNGUI", defaultValue: true);
			EnableTextMeshPro = PluginEnvironment.Current.Preferences.GetOrDefault("TextFrameworks", "EnableTextMeshPro", defaultValue: true);
			EnableTextMesh = PluginEnvironment.Current.Preferences.GetOrDefault("TextFrameworks", "EnableTextMesh", defaultValue: false);
			EnableFairyGUI = PluginEnvironment.Current.Preferences.GetOrDefault("TextFrameworks", "EnableFairyGUI", defaultValue: true);
			MaxCharactersPerTranslation = PluginEnvironment.Current.Preferences.GetOrDefault("Behaviour", "MaxCharactersPerTranslation", 200);
			IgnoreWhitespaceInDialogue = PluginEnvironment.Current.Preferences.GetOrDefault("Behaviour", "IgnoreWhitespaceInDialogue", defaultValue: true);
			...
			CopyToClipboard = PluginEnvironment.Current.Preferences.GetOrDefault("Behaviour", "CopyToClipboard", defaultValue: false);
			...
			EnableUIResizing = PluginEnvironment.Current.Preferences.GetOrDefault("Behaviour", "EnableUIResizing", defaultValue: true);
			EnableBatching = PluginEnvironment.Current.Preferences.GetOrDefault("Behaviour", "EnableBatching", defaultValue: true);
			UseStaticTranslations = PluginEnvironment.Current.Preferences.GetOrDefault("Behaviour", "UseStaticTranslations", defaultValue: true);
			OverrideFont = PluginEnvironment.Current.Preferences.GetOrDefault("Behaviour", "OverrideFont", string.Empty);
			OverrideFontSize = PluginEnvironment.Current.Preferences.GetOrDefault<int?>("Behaviour", "OverrideFontSize", null);
			...
			ForceUIResizing = PluginEnvironment.Current.Preferences.GetOrDefault("Behaviour", "ForceUIResizing", defaultValue: false);
			IgnoreTextStartingWith = (from x in PluginEnvironment.Current.Preferences.GetOrDefault("Behaviour", "IgnoreTextStartingWith", "\\u180e;")?.Split(new char[1] { ';' }, StringSplitOptions.RemoveEmptyEntries)
				select JsonHelper.Unescape(x)).ToArray() ?? new string[0];
			TextGetterCompatibilityMode = PluginEnvironment.Current.Preferences.GetOrDefault("Behaviour", "TextGetterCompatibilityMode", defaultValue: false);
			GameLogTextPaths = PluginEnvironment.Current.Preferences.GetOrDefault("Behaviour", "GameLogTextPaths", string.Empty)?.Split(new char[1] { ';' }, StringSplitOptions.RemoveEmptyEntries).ToHashSet() ?? new HashSet<string>();
			GameLogTextPaths.RemoveWhere((string x) => !x.StartsWith("/"));
			...
			CacheRegexPatternResults = PluginEnvironment.Current.Preferences.GetOrDefault("Behaviour", "CacheRegexPatternResults", defaultValue: false);
			...
			CacheRegexLookups = PluginEnvironment.Current.Preferences.GetOrDefault("Behaviour", "CacheRegexLookups", defaultValue: false);
			CacheWhitespaceDifferences = PluginEnvironment.Current.Preferences.GetOrDefault("Behaviour", "CacheWhitespaceDifferences", defaultValue: false);
			GenerateStaticSubstitutionTranslations = PluginEnvironment.Current.Preferences.GetOrDefault("Behaviour", "GenerateStaticSubstitutionTranslations", defaultValue: false);
			GeneratePartialTranslations = PluginEnvironment.Current.Preferences.GetOrDefault("Behaviour", "GeneratePartialTranslations", defaultValue: false);
			EnableTranslationScoping = PluginEnvironment.Current.Preferences.GetOrDefault("Behaviour", "EnableTranslationScoping", defaultValue: true);
			EnableSilentMode = PluginEnvironment.Current.Preferences.GetOrDefault("Behaviour", "EnableSilentMode", defaultValue: true);
			BlacklistedIMGUIPlugins = (from x in PluginEnvironment.Current.Preferences.GetOrDefault("Behaviour", "BlacklistedIMGUIPlugins", string.Empty)?.Split(new char[1] { ';' }, StringSplitOptions.RemoveEmptyEntries)
				select x.Trim() into x
				where !string.IsNullOrEmpty(x)
				select x).ToHashSet(StringComparer.OrdinalIgnoreCase) ?? new HashSet<string>(StringComparer.OrdinalIgnoreCase);
			EnableTextPathLogging = PluginEnvironment.Current.Preferences.GetOrDefault("Behaviour", "EnableTextPathLogging", defaultValue: false);
			OutputUntranslatableText = PluginEnvironment.Current.Preferences.GetOrDefault("Behaviour", "OutputUntranslatableText", defaultValue: false);
			IgnoreVirtualTextSetterCallingRules = PluginEnvironment.Current.Preferences.GetOrDefault("Behaviour", "IgnoreVirtualTextSetterCallingRules", defaultValue: false);
			MaxTextParserRecursion = PluginEnvironment.Current.Preferences.GetOrDefault("Behaviour", "MaxTextParserRecursion", 1);
			HtmlEntityPreprocessing = PluginEnvironment.Current.Preferences.GetOrDefault("Behaviour", "HtmlEntityPreprocessing", defaultValue: true);
			HandleRichText = PluginEnvironment.Current.Preferences.GetOrDefault("Behaviour", "HandleRichText", defaultValue: true);
			EnableTranslationHelper = PluginEnvironment.Current.Preferences.GetOrDefault("Behaviour", "EnableTranslationHelper", defaultValue: false);
			ForceMonoModHooks = PluginEnvironment.Current.Preferences.GetOrDefault("Behaviour", "ForceMonoModHooks", defaultValue: false);
			...
			OutputTooLongText = PluginEnvironment.Current.Preferences.GetOrDefault("Behaviour", "OutputTooLongText", defaultValue: false);
			TemplateAllNumberAway = PluginEnvironment.Current.Preferences.GetOrDefault("Behaviour", "TemplateAllNumberAway", defaultValue: false);
			ReloadTranslationsOnFileChange = PluginEnvironment.Current.Preferences.GetOrDefault("Behaviour", "ReloadTranslationsOnFileChange", defaultValue: false);
			...
			CacheParsedTranslations = PluginEnvironment.Current.Preferences.GetOrDefault("Behaviour", "CacheParsedTranslations", defaultValue: false);
			...
			TexturesPath = Path.Combine(PluginEnvironment.Current.TranslationPath, TextureDirectory).Parameterize();
			...
			EnableTextureDumping = PluginEnvironment.Current.Preferences.GetOrDefault("Texture", "EnableTextureDumping", defaultValue: false);
			EnableTextureToggling = PluginEnvironment.Current.Preferences.GetOrDefault("Texture", "EnableTextureToggling", defaultValue: false);
			EnableTextureScanOnSceneLoad = PluginEnvironment.Current.Preferences.GetOrDefault("Texture", "EnableTextureScanOnSceneLoad", defaultValue: false);
			EnableSpriteRendererHooking = PluginEnvironment.Current.Preferences.GetOrDefault("Texture", "EnableSpriteRendererHooking", defaultValue: false);
			LoadUnmodifiedTextures = PluginEnvironment.Current.Preferences.GetOrDefault("Texture", "LoadUnmodifiedTextures", defaultValue: false);
			DetectDuplicateTextureNames = PluginEnvironment.Current.Preferences.GetOrDefault("Texture", "DetectDuplicateTextureNames", defaultValue: false);
			DuplicateTextureNames = PluginEnvironment.Current.Preferences.GetOrDefault("Texture", "DuplicateTextureNames", string.Empty)?.Split(new char[1] { ';' }, StringSplitOptions.RemoveEmptyEntries).ToHashSet() ?? new HashSet<string>();
			EnableLegacyTextureLoading = PluginEnvironment.Current.Preferences.GetOrDefault("Texture", "EnableLegacyTextureLoading", defaultValue: false);
			...
			CacheTexturesInMemory = PluginEnvironment.Current.Preferences.GetOrDefault("Texture", "CacheTexturesInMemory", defaultValue: true);
			EnableSpriteHooking = PluginEnvironment.Current.Preferences.GetOrDefault("Texture", "EnableSpriteHooking", defaultValue: false);
			...
			EnableTextAssetRedirector = PluginEnvironment.Current.Preferences.GetOrDefault("ResourceRedirector", "EnableTextAssetRedirector", defaultValue: false);
			LogAllLoadedResources = PluginEnvironment.Current.Preferences.GetOrDefault("ResourceRedirector", "LogAllLoadedResources", defaultValue: false);
			EnableDumping = PluginEnvironment.Current.Preferences.GetOrDefault("ResourceRedirector", "EnableDumping", defaultValue: false);
			CacheMetadataForAllFiles = PluginEnvironment.Current.Preferences.GetOrDefault("ResourceRedirector", "CacheMetadataForAllFiles", defaultValue: true);
			...
			DisableCertificateValidation = PluginEnvironment.Current.Preferences.GetOrDefault("Http", "DisableCertificateValidation", defaultValue: true);
			Width = PluginEnvironment.Current.Preferences.GetOrDefault("TranslationAggregator", "Width", 400f);
			Height = PluginEnvironment.Current.Preferences.GetOrDefault("TranslationAggregator", "Height", 100f);
			EnabledTranslators = PluginEnvironment.Current.Preferences.GetOrDefault("TranslationAggregator", "EnabledTranslators", string.Empty)?.Split(new char[1] { ';' }, StringSplitOptions.RemoveEmptyEntries).ToHashSet() ?? new HashSet<string>();
			EnableConsole = PluginEnvironment.Current.Preferences.GetOrDefault("Debug", "EnableConsole", defaultValue: false);
			EnableMigrations = PluginEnvironment.Current.Preferences.GetOrDefault("Migrations", "Enable", defaultValue: true);
			...
			TranslationsPath = Path.Combine(PluginEnvironment.Current.TranslationPath, TranslationDirectory.UseCorrectDirectorySeparators()).Parameterize();
			...
			string migrationsTag = (PluginEnvironment.Current.Preferences["Migrations"]["Tag"].Value = "5.5");
			MigrationsTag = migrationsTag;
			Save();
		}
		catch (Exception e4)
		{
			XuaLogger.AutoTranslator.Error(e4, "An error occurred during configuration. Shutting plugin down.");
			IsShutdown = true;
		}
	}
