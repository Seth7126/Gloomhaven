...
internal class ZipEntry : ICloneable
{
	...
	public bool RequiresZip64 => size > 4294967295L || compressedSize > 4294967295L;
	...
	public long Size
	{
		get
		{
			return ((known & KNOWN_SIZE) == 0) ? (-1L) : ((long)size);
		}
		set
		{
			if ((value & -4294967296L) != 0L)
	...
	public long CompressedSize
	{
		get
		{
			return ((known & KNOWN_CSIZE) == 0) ? (-1L) : ((long)compressedSize);
		}
		set
		{
			if ((value & -4294967296L) != 0L)
	...
	public long Crc
	{
		get
		{
			return ((known & KNOWN_CRC) == 0) ? (-1) : ((long)crc & 0xFFFFFFFFL);
		}
		set
		{
				...
				for (int i = 0; i < extra.Length; i += num2)
				{
					...
					case 21589:
					{
						...
						if ((num3 & 1) != 0 && num2 >= 5)
						{
							...
							DateTime = (dateTime + timeSpan).ToLocalTime();
							known |= (ushort)KNOWN_TIME;
						}
						break;
					}
	...
	public bool IsFile
	{
		get
		{
			...
			if (flag && (known & KNOWN_EXTERN_ATTRIBUTES) != 0 && HostSystem == 0 && (ExternalFileAttributes & 8) != 0)
			{
				flag = false;
			}
			return flag;
