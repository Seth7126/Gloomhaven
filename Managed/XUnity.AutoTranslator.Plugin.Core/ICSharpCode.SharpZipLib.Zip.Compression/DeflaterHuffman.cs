...
internal class DeflaterHuffman
{
	public class Tree
	{
		...
		public void BuildCodes()
		{
			...
			for (int j = 0; j < numCodes; j++)
			{
				...
				if (num3 > 0)
				{
					codes[j] = BitReverse(array[num3 - 1]);
		...
		private void BuildLength(int[] childs)
		{
			...
			for (int num4 = num - 1; num4 >= 0; num4--)
			{
				if (childs[2 * num4 + 1] != -1)
				{
					...
					int num6 = num5;
					array[childs[2 * num4 + 1]] = num6;
					array[childs[2 * num4]] = num6;
				}
				else
				{
					int num7 = array[num4];
			...
			while (true)
			{
				if (bl_counts[--num8] != 0)
				{
					do
					{
						bl_counts[num8]--;
						bl_counts[++num8]++;
						num3 -= 1 << ((maxLength - 1 - num8) & 0x1F);
					}
					while (num3 > 0 && num8 < maxLength - 1);
					if (num3 <= 0)
					{
						break;
					}
			...
			{
				int num11 = bl_counts[num10 - 1];
				while (num11 > 0)
				{
					int num12 = 2 * childs[num9++];
					if (childs[num12 + 1] == -1)
					{
						length[childs[num12]] = (byte)num10;
						num11--;
					}
				}
			}
	...
	{
		pending.WriteBits(0 + (lastBlock ? 1 : 0), 3);
		pending.AlignToByte();
