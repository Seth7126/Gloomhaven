...
public abstract class AssetLoadedHandlerBaseV2<TAsset> where TAsset : Object
{
	...
	private void Handle(IAssetOrResourceLoadedContext context)
	{
		...
		if ((CheckDirectory && RedirectedDirectory.DirectoryExists(text)) || (!CheckDirectory && RedirectedDirectory.FileExists(text)))
		{
			try
			{
