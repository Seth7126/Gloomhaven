...
internal class TarHeader : ICloneable
{
	...
	public DateTime ModTime
	{
		...
		set
		{
			...
			ref DateTime reference = ref modTime;
			reference = new DateTime(value.Year, value.Month, value.Day, value.Hour, value.Minute, value.Second);
		}
	}
	...
	private static int MakeCheckSum(byte[] buf)
	{
		...
		for (int k = 148 + CHKSUMLEN; k < buf.Length; k++)
		{
			num += buf[k];
		}
		return num;
