...
internal class TarInputStream : Stream
{
	...
	public override int Read(byte[] outputBuffer, int offset, int count)
	{
		...
		if (readBuf != null)
		{
			int num3 = (int)((num2 <= readBuf.Length) ? num2 : readBuf.Length);
		...
		while (num2 > 0)
		{
			...
			if (num6 > num5)
			{
				...
				readBuf = new byte[num6 - num5];
				Array.Copy(array, num5, readBuf, 0, num6 - num5);
			}
			else
			{
				num5 = num6;
				Array.Copy(array, 0, outputBuffer, offset, num6);
			}
			num += num5;
			num2 -= num5;
			offset += num5;
