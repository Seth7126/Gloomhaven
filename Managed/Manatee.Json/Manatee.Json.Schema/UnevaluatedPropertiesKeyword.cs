...
public class UnevaluatedPropertiesKeyword : IJsonSchemaKeyword, IJsonSerializable, IEquatable<IJsonSchemaKeyword>, IEquatable<UnevaluatedPropertiesKeyword>
{
	public static string ErrorTemplate { get; set; } = "Properties {{properties}} were not covered by `properties`, `patternProperties`, and `additionalProperties` failed validation of the local subschema.";
	...
	public SchemaValidationResults Validate(SchemaValidationContext context)
	{
		...
		JsonObject jsonObject = source.Where<KeyValuePair<string, JsonValue>>((KeyValuePair<string, JsonValue> kvp) => !context.EvaluatedPropertyNames.Contains(kvp.Key)).ToJson();
		if (jsonObject.Count == 0)
		{
			Log.Schema(() => "All properties have been evaluated");
			return schemaValidationResults;
		}
		Log.Schema(() => (context.EvaluatedPropertyNames.Count != 0) ? $"Properties {context.EvaluatedPropertyNames.ToJson()} have been evaluated; skipping these" : "No properties have been evaluated; process all");
