...
internal class ExpressionFilterParser : IJsonPathParser
{
	...
	public bool TryParse(string source, ref int index, [NotNullWhen(true)] ref JsonPath? path, [NotNullWhen(false)] out string? errorMessage)
	{
		...
		index += 2;
		if (!JsonPathExpressionParser.TryParse(source, ref index, out Expression<bool, JsonValue> expr, out errorMessage))
		{
			return false;
		}
		...
		index++;
		path.Operators.Add(new ArrayOperator(new FilterExpressionQuery(expr)));
		return true;
	}
}
