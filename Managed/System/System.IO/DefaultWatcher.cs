	...
	private void IterateAndModifyFilesData(DefaultWatcherData data, string directory, bool dispatch, string[] files)
	{
		...
		foreach (string text in files)
		{
			if (!data.Files.TryGetValue(text, out var value2))
			{
				...
				if (dispatch)
				{
					DispatchEvents(data.FSW, FileAction.Added, Path.GetRelativePath(directory, text));
				}
			}
		...
		foreach (KeyValuePair<string, FileData> file2 in data.Files)
		{
			...
			if (value3.NotExists)
			{
				...
				list.Add(key);
				DispatchEvents(data.FSW, FileAction.Removed, Path.GetRelativePath(value3.Directory, key));
			}
		}
		if (list != null)
		{
		...
		foreach (KeyValuePair<string, FileData> file3 in data.Files)
		{
			...
			catch
			{
				...
				list.Add(key2);
				DispatchEvents(data.FSW, FileAction.Removed, Path.GetRelativePath(value4.Directory, key2));
				continue;
			}
			if (creationTime != value4.CreationTime || lastWriteTime != value4.LastWriteTime)
			{
				value4.CreationTime = creationTime;
				value4.LastWriteTime = lastWriteTime;
				DispatchEvents(data.FSW, FileAction.Modified, Path.GetRelativePath(value4.Directory, key2));
			}
		}
		if (list == null)
		{
