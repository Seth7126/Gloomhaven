...
public class PlayerAction : OneAxisInputControl
{
	...
	public void RemoveBinding(InputControlType inputControlType)
	{
		for (int i = 0; i < regularBindings.Count; i++)
		{
			if (regularBindings[i] is DeviceBindingSource deviceBindingSource && deviceBindingSource.Control == inputControlType)
			{
				deviceBindingSource.BoundTo = null;
				triggerBindingChanged = true;
				break;
			}
		}
	...
	public void RemoveBinding(Mouse mouse)
	{
		for (int i = 0; i < regularBindings.Count; i++)
		{
			if (regularBindings[i] is MouseBindingSource mouseBindingSource && mouseBindingSource.Control == mouse)
			{
				mouseBindingSource.BoundTo = null;
				triggerBindingChanged = true;
				break;
			}
		}
	...
	public void RemoveBinding(Key key)
	{
		for (int i = 0; i < regularBindings.Count; i++)
		{
			if (regularBindings[i] is KeyBindingSource { Control: var control } keyBindingSource && control.GetInclude(0) == key)
			{
				keyBindingSource.BoundTo = null;
				triggerBindingChanged = true;
				break;
			}
		}
	...
	public void RemoveBinding(KeyCombo keyCombo)
	{
		for (int i = 0; i < regularBindings.Count; i++)
		{
			if (regularBindings[i] is KeyBindingSource keyBindingSource && keyBindingSource.Control == keyCombo)
			{
				keyBindingSource.BoundTo = null;
				triggerBindingChanged = true;
				break;
