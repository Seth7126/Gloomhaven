...
public class InControlInputModule : PointerInputModuleExtended
{
	public enum Button
	{
		Action1 = 19,
	...
	private MoveActionSourceType _currentActionSourceType;

	private bool skipNextSubmit;

	private bool skipNextCancel;

	private bool disableMoveEvents;

	public PlayerAction SubmitAction { get; set; }

	public PlayerAction CancelAction { get; set; }

	public List<MoveActionModel> MoveActions { get; }

	public List<MoveActionModel> DefaultMoveDirections { get; }
	...
	public event Action<InputVectorEventData> InputVectorEvent;

	public event Action<Vector2> PreUnityInputVectorEvent;

	protected InControlInputModule()
	{
		direction = new TwoAxisInputControl
		{
			StateThreshold = analogMoveThreshold
		};
		DefaultMoveDirections = new List<MoveActionModel>
		{
			new MoveActionModel(direction, MoveActionSourceType.DPad)
		};
		MoveActions = new List<MoveActionModel>();
	}
	...
	public void AddMoveAction(TwoAxisInputControl twoAxisAction, MoveActionSourceType moveActionSourceType)
	{
		MoveActions.Add(new MoveActionModel(twoAxisAction, moveActionSourceType));
	}
	...
	public override void Process()
	{
		...
		if (base.eventSystem.sendNavigationEvents && !SendVectorEventToSelectedObject())
		{
	...
	public void SkipNextSubmitAction()
	{
		skipNextSubmit = true;
	}

	public void SkipNextCancelAction()
	{
		skipNextCancel = true;
	}

	public void DisableMoveEvents()
	{
		disableMoveEvents = true;
	}

	public void EnableMoveEvents()
	{
	...
	{
		int countTouch = InputSystemUtilities.GetCountTouch();
		TouchState[] touches = InputSystemUtilities.GetTouches();
		for (int i = 0; i < countTouch; i++)
		{
			TouchState touchState = touches[i];
			if (CheckIsCorrectTouch(touchState))
			{
				bool pressed;
				bool released;
				PointerEventData touchPointerEventData = GetTouchPointerEventData(touchState, out pressed, out released);
	...
	private bool SendButtonEventToSelectedObject()
	{
		...
		if (SubmitWasPressed && !skipNextSubmit)
		{
		...
		if (CancelWasPressed && !skipNextCancel)
		{
		...
		skipNextCancel = false;
		return baseEventData.used;
	}

	private bool SendVectorEventToSelectedObject()
	{
		if (!VectorWasPressed || disableMoveEvents)
		{
			return false;
		}
		...
		if (axisEventData.moveDir != MoveDirection.None)
		{
			...
			{
				this.PreUnityInputVectorEvent?.Invoke(thisVectorState);
				ExecuteEvents.Execute(base.eventSystem.currentSelectedGameObject, axisEventData, ExecuteEvents.moveHandler);
			}
			this.InputVectorEvent?.Invoke(new InputVectorEventData(_currentActionSourceType, thisVectorState));
	...
	private void UpdateInputState()
	{
		...
		_currentActionSourceType = MoveActionSourceType.None;
		UpdateInputStateInternal(MoveActions);
		if (!MoveActions.Any((MoveActionModel x) => x.TwoAxisInputControl != null))
		{
			UpdateInputStateInternal(DefaultMoveDirections);
		}
	}

	private void UpdateInputStateInternal(List<MoveActionModel> moveActionModels)
	{
		foreach (MoveActionModel moveActionModel in moveActionModels)
		{
			if (moveActionModel.TwoAxisInputControl != null)
			{
				bool flag = false;
				if (Utility.AbsoluteIsOverThreshold(moveActionModel.TwoAxisInputControl.X, analogMoveThreshold))
				{
					thisVectorState.x = Mathf.Sign(moveActionModel.TwoAxisInputControl.X);
					flag = true;
				}
				if (Utility.AbsoluteIsOverThreshold(moveActionModel.TwoAxisInputControl.Y, analogMoveThreshold))
				{
					thisVectorState.y = Mathf.Sign(moveActionModel.TwoAxisInputControl.Y);
					flag = true;
				}
				if (flag)
				{
					_currentActionSourceType = moveActionModel.SourceType;
					break;
				}
			}
		}
		...
		thisSubmitState = SubmitAction?.IsPressed ?? SubmitButton.IsPressed;
		lastCancelState = thisCancelState;
		thisCancelState = CancelAction?.IsPressed ?? CancelButton.IsPressed;
	}
