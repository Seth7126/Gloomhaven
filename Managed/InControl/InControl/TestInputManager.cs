...
public class TestInputManager : MonoBehaviour
{
	...
	private void OnEnable()
	{
		...
		{
			LogUtils.Log("Attached: " + inputDevice.Name);
		};
		InputManager.OnDeviceDetached += delegate(InputDevice inputDevice)
		{
			LogUtils.Log("Detached: " + inputDevice.Name);
		};
		InputManager.OnActiveDeviceChanged += delegate(InputDevice inputDevice)
		{
			LogUtils.Log("Active device changed to: " + inputDevice.Name);
		};
	...
	private void OnGUI()
	{
		...
		foreach (InputDevice device in InputManager.Devices)
		{
			...
			{
				GUI.Label(new Rect(num2, num3, num2 + num, num3 + 10), "Profile: " + nativeInputDevice.ProfileName, style);
				num3 += 15;
				...
				num3 += 15;
				text2 = $"DRV: {nativeInputDevice.Info.driverType}, TSP: {nativeInputDevice.Info.transportType}";
				GUI.Label(new Rect(num2, num3, num2 + num, num3 + 10), text2, style);
				num3 += 15;
			}
			num3 += 15;
