...
public class InputDevice
{
	...
	public bool AnyControlIsPressed
	{
		get
		{
			int count = Controls.Count;
	...
	public InputDevice(string name, bool rawSticks)
	{
		...
		Passive = false;
		ControlsByTarget = new InputControl[531];
	...
	private void ProcessRightStick(ulong updateTick, float deltaTime)
	{
		...
		float y = Utility.ValueFromSides(RightStickDown.NextRawValue, RightStickUp.NextRawValue);
		Vector2 vector;
	...
	private void ProcessDPad(ulong updateTick, float deltaTime)
	{
		...
		float y = Utility.ValueFromSides(DPadDown.NextRawValue, DPadUp.NextRawValue);
		Vector2 vector;
