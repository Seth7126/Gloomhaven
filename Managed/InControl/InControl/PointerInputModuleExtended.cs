...
public abstract class PointerInputModuleExtended : BaseInputModule
{
	...
	protected Dictionary<int, bool> m_isReleaseTouch = new Dictionary<int, bool>();
	...
	protected bool CheckIsCorrectTouch(TouchState input)
	{
		int touchId = input.touchId;
		UnityEngine.InputSystem.TouchPhase phase = input.phase;
		bool value = false;
		m_isReleaseTouch.TryGetValue(touchId, out value);
		if (phase == UnityEngine.InputSystem.TouchPhase.None || (value && phase == UnityEngine.InputSystem.TouchPhase.Ended))
		{
			return false;
		}
		return true;
	...
	protected PointerEventData GetTouchPointerEventData(TouchState input, out bool pressed, out bool released)
	{
		PointerEventData data;
		bool pointerData = GetPointerData(input.touchId, out data, create: true);
		data.Reset();
		pressed = pointerData || input.phase == UnityEngine.InputSystem.TouchPhase.Began;
		released = input.phase == UnityEngine.InputSystem.TouchPhase.Canceled || input.phase == UnityEngine.InputSystem.TouchPhase.Ended;
		if (released)
		{
			m_isReleaseTouch[input.touchId] = true;
		}
		else
		{
			m_isReleaseTouch[input.touchId] = false;
		}
		if (pointerData)
		{
		...
		if (input.phase == UnityEngine.InputSystem.TouchPhase.Canceled)
		{
	...
	{
		MouseButton mouseButtonWithID = InputSystemUtilities.GetMouseButtonWithID(buttonId);
		bool mouseButtionDown = InputSystemUtilities.GetMouseButtionDown(mouseButtonWithID);
		bool mouseButtionUp = InputSystemUtilities.GetMouseButtionUp(mouseButtonWithID);
		if (mouseButtionDown && mouseButtionUp)
		{
	...
	protected virtual MouseState GetMousePointerEventData(int id)
	{
		...
		if (pointerData)
		{
			data.position = InputSystemUtilities.GetMousePosition();
		}
		Vector2 vector = InputSystemUtilities.GetMousePosition();
		...
		data.scrollDelta = InputSystemUtilities.GetMouseScroll();
		data.button = PointerEventData.InputButton.Left;
		base.eventSystem.RaycastAll(data, m_RaycastResultCache);
		RaycastResult pointerCurrentRaycast = BaseInputModule.FindFirstRaycast(m_RaycastResultCache);
		data.pointerCurrentRaycast = pointerCurrentRaycast;
