...
public sealed class PlayerDataStorageInterface : Handle
{
	...
	public PlayerDataStorageFileTransferRequest ReadFile(ref ReadFileOptions readOptions, object clientData, OnReadFileCompleteCallback completionCallback)
	{
		...
		IntPtr intPtr = Bindings.EOS_PlayerDataStorage_ReadFile(base.InnerHandle, ref readOptions2, clientDataAddress, onReadFileCompleteCallbackInternal);
	...
	public PlayerDataStorageFileTransferRequest WriteFile(ref WriteFileOptions writeOptions, object clientData, OnWriteFileCompleteCallback completionCallback)
	{
		...
		IntPtr intPtr = Bindings.EOS_PlayerDataStorage_WriteFile(base.InnerHandle, ref writeOptions2, clientDataAddress, onWriteFileCompleteCallbackInternal);
	...
	{
		if (Helper.TryGetStructCallback<FileTransferProgressCallbackInfoInternal, OnFileTransferProgressCallback, FileTransferProgressCallbackInfo>(ref data, out var callback, out var _))
		{
			Helper.Get<FileTransferProgressCallbackInfoInternal, FileTransferProgressCallbackInfo>(ref data, out var to);
			callback(ref to);
		}
	}
	...
	{
		if (Helper.TryGetStructCallback<ReadFileDataCallbackInfoInternal, OnReadFileDataCallback, ReadFileDataCallbackInfo>(ref data, out var callback, out var _))
		{
			Helper.Get<ReadFileDataCallbackInfoInternal, ReadFileDataCallbackInfo>(ref data, out var to);
			return callback(ref to);
		}
	...
	{
		if (Helper.TryGetStructCallback<WriteFileDataCallbackInfoInternal, OnWriteFileDataCallback, WriteFileDataCallbackInfo>(ref data, out var callback, out var _))
		{
			Helper.Get<WriteFileDataCallbackInfoInternal, WriteFileDataCallbackInfo>(ref data, out var to);
			ArraySegment<byte> outDataBuffer2;
			WriteResult result = callback(ref to, out outDataBuffer2);
