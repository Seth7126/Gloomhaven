...
public sealed class P2PInterface : Handle
{
	...
	{
		ReceivePacketOptionsInternal options2 = default(ReceivePacketOptionsInternal);
		options2.Set(ref options);
		...
		outBytesWritten = 0u;
		IntPtr value = Helper.AddPinnedBuffer(outData);
		Result result = Bindings.EOS_P2P_ReceivePacket(base.InnerHandle, ref options2, ref outPeerId2, ref outSocketId2, ref outChannel, value, ref outBytesWritten);
		Helper.Dispose(ref options2);
		Helper.Get(outPeerId2, out outPeerId);
		Helper.Get<SocketIdInternal, SocketId>(ref outSocketId2, out outSocketId);
	...
	{
		if (Helper.TryGetAndRemoveCallback<OnIncomingConnectionRequestInfoInternal, OnIncomingConnectionRequestCallback, OnIncomingConnectionRequestInfo>(ref data, out var callback, out var callbackInfo))
		{
	...
	{
		if (Helper.TryGetAndRemoveCallback<OnIncomingPacketQueueFullInfoInternal, OnIncomingPacketQueueFullCallback, OnIncomingPacketQueueFullInfo>(ref data, out var callback, out var callbackInfo))
		{
			callback(ref callbackInfo);
		}
	}
	...
	{
		if (Helper.TryGetAndRemoveCallback<OnPeerConnectionEstablishedInfoInternal, OnPeerConnectionEstablishedCallback, OnPeerConnectionEstablishedInfo>(ref data, out var callback, out var callbackInfo))
		{
			callback(ref callbackInfo);
		}
	}
	...
	{
		if (Helper.TryGetAndRemoveCallback<OnPeerConnectionInterruptedInfoInternal, OnPeerConnectionInterruptedCallback, OnPeerConnectionInterruptedInfo>(ref data, out var callback, out var callbackInfo))
		{
			callback(ref callbackInfo);
		}
	}
	...
	{
		if (Helper.TryGetAndRemoveCallback<OnRemoteConnectionClosedInfoInternal, OnRemoteConnectionClosedCallback, OnRemoteConnectionClosedInfo>(ref data, out var callback, out var callbackInfo))
		{
			callback(ref callbackInfo);
		}
	}
