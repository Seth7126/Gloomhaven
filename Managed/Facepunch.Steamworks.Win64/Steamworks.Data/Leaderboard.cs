...
public struct Leaderboard
{
	...
	public async Task<LeaderboardEntry[]> GetScoresForUsersAsync(SteamId[] users)
	{
		...
		LeaderboardScoresDownloaded_t? r = await SteamUserStats.Internal.DownloadLeaderboardEntriesForUsers(Id, users, users.Length);
		if (!r.HasValue)
		{
			return null;
		}
	...
	public async Task<LeaderboardEntry[]> GetScoresAsync(int count, int offset = 1)
	{
		...
		LeaderboardScoresDownloaded_t? r = await SteamUserStats.Internal.DownloadLeaderboardEntries(Id, LeaderboardDataRequest.Global, offset, offset + count - 1);
		if (!r.HasValue)
		{
			return null;
		}
