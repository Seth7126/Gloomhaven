	...
	public IEnumerable<Item> Entries
	{
		get
		{
			...
			for (uint i = 0u; i < ResultCount; i++)
			{
				...
				{
					uint keyValueTagsCount = SteamUGC.Internal.GetQueryUGCNumKeyValueTags(Handle, i);
					item.KeyValueTags = new Dictionary<string, string>((int)keyValueTagsCount);
					for (uint j = 0u; j < keyValueTagsCount; j++)
					{
						if (SteamUGC.Internal.GetQueryUGCKeyValueTag(Handle, i, j, out var key, out var value))
						{
				...
				if (ReturnsMetadata)
				{
					if (SteamUGC.Internal.GetQueryUGCMetadata(Handle, i, out var metadata))
					{
				...
				{
					PublishedFileId[] children = new PublishedFileId[numChildren];
				...
				{
					uint previewsCount = SteamUGC.Internal.GetQueryUGCNumAdditionalPreviews(Handle, i);
					if (previewsCount != 0)
					{
						item.AdditionalPreviews = new UgcAdditionalPreview[previewsCount];
						for (uint j2 = 0u; j2 < previewsCount; j2++)
						{
							ItemPreviewType previewType = ItemPreviewType.Image;
							if (SteamUGC.Internal.GetQueryUGCAdditionalPreview(Handle, i, j2, out var previewUrlOrVideo, out var originalFileName, ref previewType))
							{
								item.AdditionalPreviews[j2] = new UgcAdditionalPreview(previewUrlOrVideo, originalFileName, previewType);
							}
							previewUrlOrVideo = null;
