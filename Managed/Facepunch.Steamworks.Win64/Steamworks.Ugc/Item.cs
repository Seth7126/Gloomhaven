...
public struct Item
{
	...
	{
		ResultPage? file = await Query.All.WithFileId(id).WithLongDescription(b: true).GetPageAsync(1);
		if (!file.HasValue)
		{
			return null;
		}
	...
	internal static Item From(SteamUGCDetails_t details)
	{
		return new Item
		{
			...
			Tags = details.TagsUTF8().ToLower().Split(new char[1] { ',' }, StringSplitOptions.RemoveEmptyEntries)
		};
	}
	...
	{
		AddUGCDependencyResult_t? r = await SteamUGC.Internal.AddDependency(Id, child);
		return r.HasValue && r.GetValueOrDefault().Result == Result.OK;
	}

	public async Task<bool> RemoveDependency(PublishedFileId child)
	{
		RemoveUGCDependencyResult_t? r = await SteamUGC.Internal.RemoveDependency(Id, child);
		return r.HasValue && r.GetValueOrDefault().Result == Result.OK;
	}
}
