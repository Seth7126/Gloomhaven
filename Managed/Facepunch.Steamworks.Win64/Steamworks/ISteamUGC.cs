...
internal class ISteamUGC : SteamInterface
{
	...
	{
		return SteamAPI_SteamGameServerUGC_v015();
	}
	...
	internal bool GetQueryUGCTag(UGCQueryHandle_t handle, uint index, uint indexTag, out string pchValue)
	{
		...
		bool result = _GetQueryUGCTag(Self, handle, index, indexTag, intPtr, 32768u);
	...
	{
		IntPtr intPtr = Helpers.TakeMemory();
		bool result = _GetQueryUGCTagDisplayName(Self, handle, index, indexTag, intPtr, 32768u);
		pchValue = Helpers.MemoryToString(intPtr);
		return result;
	}
