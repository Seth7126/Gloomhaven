...
public class SteamFriends : SteamClientClass<SteamFriends>
{
	...
	private static void OnFriendChatMessage(GameConnectedFriendChatMsg_t data)
	{
		if (SteamFriends.OnChatMessage != null)
		{
			...
			if (Internal.GetFriendMessage(data.SteamIDUser, data.MessageID, intPtr, 32768, ref peChatEntryType) != 0 || peChatEntryType != ChatEntryType.Invalid)
			{
				string arg2 = peChatEntryType.ToString();
				string arg3 = Helpers.MemoryToString(intPtr);
	...
	private static void OnGameConnectedClanChatMessage(GameConnectedClanChatMsg_t data)
	{
		if (SteamFriends.OnClanChatMessage != null)
		{
			...
			if (Internal.GetClanChatMessage(data.SteamIDClanChat, data.MessageID, intPtr, 32768, ref peChatEntryType, ref psteamidChatter) != 0 || peChatEntryType != ChatEntryType.Invalid)
			{
				string arg2 = peChatEntryType.ToString();
				string arg3 = Helpers.MemoryToString(intPtr);
	...
	{
		JoinClanChatRoomCompletionResult_t? result = await Internal.JoinClanChatRoom(chatId);
		if (!result.HasValue)
		{
			return false;
		}
