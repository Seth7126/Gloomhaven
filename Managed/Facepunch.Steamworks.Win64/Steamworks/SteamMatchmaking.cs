...
public class SteamMatchmaking : SteamClientClass<SteamMatchmaking>
{
	...
	private static void OnLobbyChatMessageRecievedAPI(LobbyChatMsg_t callback)
	{
		...
		int lobbyChatEntry = Internal.GetLobbyChatEntry(callback.SteamIDLobby, (int)callback.ChatID, ref pSteamIDUser, intPtr, 32768, ref peChatEntryType);
		if (lobbyChatEntry > 0)
		{
			SteamMatchmaking.OnChatMessage?.Invoke(new Lobby(callback.SteamIDLobby), new Friend(pSteamIDUser), Helpers.MemoryToString(intPtr));
		}
	}
