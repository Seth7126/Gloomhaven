...
public class LeanAudio
{
	...
	private static int createAudioWave(AnimationCurve volume, AnimationCurve frequency, LeanAudioOptions options)
	{
		...
		for (int i = 0; i < PROCESSING_ITERATIONS_MAX; i++)
		{
			...
			{
				LogUtils.LogError("LeanAudio has reached it's processing cap. To avoid this error increase the number of iterations ex: LeanAudio.PROCESSING_ITERATIONS_MAX = " + PROCESSING_ITERATIONS_MAX * 2);
				break;
			}
			int num7 = num / 2;
			num2 += num3;
	...
	private static AudioClip createAudioFromWave(int waveLength, LeanAudioOptions options)
	{
		...
		for (int i = 0; i < array.Length; i++)
		{
			...
			if (options.modulation != null)
			{
				...
				{
					float num11 = Mathf.Abs(Mathf.Sin(1.5708f + num6 * (1f / options.modulation[j][0]) * MathF.PI));
