...
public class LTDescr
{
	...
	public LTDescr cancel(GameObject gameObject)
	{
		if (debug)
		{
			LogUtils.Log("canceling id:" + _id + " this.uniqueId:" + uniqueId + " go:" + trans.gameObject);
		}
	...
	public bool updateInternal()
	{
		...
		if (delay <= 0f && num != 0f)
		{
			...
			bool flag = ((num > 0f) ? (passed >= time) : (passed <= 0f));
			if (debug)
			{
				LogUtils.Log("tween:" + this?.ToString() + " dt:" + dt + " ratio:" + ratioPassed + " time:" + passed + " finished:" + flag);
			}
			if (flag)
			{
				loopCount--;
				...
				flag = loopCount == 0 || loopType == LeanTweenType.once;
		...
		else
		{
			...
			if (debug)
			{
				LogUtils.Log("tween:" + this?.ToString() + " dt:" + dt + " delay:" + delay);
			}
		}
		return false;
	}
	...
	public LTDescr setDirection(float direction)
	{
		...
		{
			LogUtils.LogWarning("You have passed an incorrect direction of '" + direction + "', direction must be -1f or 1f");
			return this;
		}
