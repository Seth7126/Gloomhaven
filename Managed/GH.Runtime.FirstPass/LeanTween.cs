...
public class LeanTween : MonoBehaviour
{
	...
	public static void update()
	{
		...
		for (int j = 0; j < finishedCnt; j++)
		{
			...
			if (tween.id == tweensFinishedIds[j])
			{
				if (tween.debug)
				{
					LogUtils.Log("removing tween from update:" + tween);
				}
				removeTween(LeanTween.j, resetCallbacks: false);
	...
	public static void removeTween(int i, bool resetCallbacks = true)
	{
		if (!tweens[i].toggle)
		{
			return;
	...
	public static void cancel(GameObject gameObject, string debugInfo = null)
	{
		cancel(gameObject, callOnComplete: false, debugInfo);
	}

	public static void cancel(GameObject gameObject, bool callOnComplete, string debugInfo = null)
	{
		init();
		...
		for (int i = 0; i <= tweenMaxSearch; i++)
		{
			if (tweens[i].toggle && tweens[i].trans == transform)
			{
				...
				if (tweens[i].debug)
				{
					LogUtils.Log($"cancel because {gameObject.name} extraInfo: {(string.IsNullOrEmpty(debugInfo) ? string.Empty : debugInfo)} {tweens[i]}");
				}
				removeTween(i);
			}
		}
	...
	public static void cancel(RectTransform rect, string debugInfo = null)
	{
		cancel(rect.gameObject, callOnComplete: false, debugInfo);
	}
	...
	public static void cancel(int uniqueId, string debugInfo = null)
	{
		cancel(uniqueId, callOnComplete: false, debugInfo);
	}

	public static void cancel(int uniqueId, bool callOnComplete, string debugInfo = null)
	{
		if (uniqueId < 0)
		{
			return;
		...
		if (num > tweens.Length - 1)
		{
			...
			for (int i = 0; i < maxSequences; i++)
			{
				...
				{
					int num4 = lTSeq.current.tween.uniqueId & 0xFFFF;
					if (tweens[num4].debug)
					{
						LogUtils.Log($"cancel by id seq {uniqueId} extraInfo: {(string.IsNullOrEmpty(debugInfo) ? string.Empty : debugInfo)} {tweens[num4]}");
					}
					removeTween(num4);
				}
		...
		else if (tweens[num].counter == num2)
		{
			...
			if (tweens[num].debug)
			{
				LogUtils.Log($"cancel by id {uniqueId} extraInfo: {(string.IsNullOrEmpty(debugInfo) ? string.Empty : debugInfo)} {tweens[num]}");
			}
			removeTween(num);
		}
	}
	...
	{
		LogUtils.LogError("error this function is no longer used");
		return null;
	}
	...
	public static void addListener(GameObject caller, int eventId, Action<LTEvent> callback)
	{
		...
		LogUtils.LogError("You ran out of areas to add listeners, consider increasing LISTENERS_MAX, ex: LeanTween.LISTENERS_MAX = " + LISTENERS_MAX * 2);
	}
