...
[Preserve]
public class SocketUdpPool : IPhotonSocket, IDisposable
{
	private Socket sock;

	private readonly object syncer = new object();

	public SocketUdpPool(PeerBase npeer)
		: base(npeer)
	{
	...
	public override bool Connect()
	{
		...
		ThreadManager.Start(DnsAndConnect);
		return true;
	}
		...
		catch (Exception ex)
		{
			if (base.State != PhotonSocketState.Disconnecting && base.State != PhotonSocketState.Disconnected)
			{
				if (ReportDebugOfLevel(DebugLevel.INFO))
				{
					...
					EnqueueDebugReturn(DebugLevel.INFO, string.Format("Cannot send to: {0}. {1} {2}\n{3}", base.ServerAddress, base.AddressResolvedAsIpv6 ? " IPv6" : string.Empty, text, ex));
				}
				if (!sock.Connected)
				{
	...
	internal void DnsAndConnect()
	{
		...
		else
		{
			...
			peerBase.OnConnect();
			ThreadManager.Start(ReceiveLoop);
		}
	}
