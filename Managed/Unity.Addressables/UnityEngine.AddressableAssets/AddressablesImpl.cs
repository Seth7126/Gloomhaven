...
internal class AddressablesImpl : IEqualityComparer<IResourceLocation>
{
	...
	internal void OnSceneUnloaded(Scene scene)
	{
		foreach (AsyncOperationHandle sceneInstance in m_SceneInstances)
		{
			...
			if (sceneLoadHandle.Result.Scene == scene)
			{
				...
				AsyncOperationHandle<SceneInstance> handle = SceneProvider.ReleaseScene(m_ResourceManager, sceneLoadHandle);
	...
	public string ResolveInternalId(string id)
	{
		...
		if (text.Length >= 260 && text.StartsWith(Application.dataPath))
		{
	...
	public AsyncOperationHandle<IResourceLocator> InitializeAsync(string runtimeDataPath, string providerSuffix = null, bool autoReleaseHandle = true)
	{
		if (hasStartedInitialization)
		{
			...
			AsyncOperationHandle<IResourceLocator> asyncOperationHandle = ResourceManager.CreateCompletedOperation(m_ResourceLocators[0].Locator, null);
			if (autoReleaseHandle)
			{
				AutoReleaseHandleOnCompletion(asyncOperationHandle);
			}
	...
	internal ResourceLocationBase CreateCatalogLocationWithHashDependencies(string catalogPath, string hashFilePath)
	{
		...
		resourceLocationBase.Data = new ProviderLoadRequestOptions
		{
		...
		if (!string.IsNullOrEmpty(hashFilePath))
		{
			...
			ResourceLocationBase resourceLocationBase2 = new ResourceLocationBase(text2, text2, typeof(TextDataProvider).FullName, typeof(string));
			resourceLocationBase2.Data = providerLoadRequestOptions.Copy();
			resourceLocationBase.Dependencies.Add(resourceLocationBase2);
		}
	...
	{
		string hashFilePath = catalogPath.Replace(".json", ".hash");
		ResourceLocationBase loc = CreateCatalogLocationWithHashDependencies(catalogPath, hashFilePath);
		...
		AsyncOperationHandle<IResourceLocator> asyncOperationHandle = InitializationOperation.LoadContentCatalog(this, loc, providerSuffix);
		if (autoReleaseHandle)
		{
			AutoReleaseHandleOnCompletion(asyncOperationHandle);
		}
	...
	public AsyncOperationHandle DownloadDependenciesAsync(object key, bool autoReleaseHandle = false)
	{
		...
		if (autoReleaseHandle)
		{
			AutoReleaseHandleOnCompletion(asyncOperationHandle2);
		}
	...
	internal bool ClearDependencyCacheForKey(object key)
	{
		...
		{
			foreach (IResourceLocation item in GatherDependenciesFromLocations((key as IResourceLocation).Dependencies))
			{
				if (item.Data is AssetBundleRequestOptions)
				{
					flag = flag && Caching.ClearAllCachedVersions((item.Data as AssetBundleRequestOptions).BundleName);
				}
			}
		}
		else if (GetResourceLocations(key, typeof(object), out locations))
		{
			foreach (IResourceLocation item2 in GatherDependenciesFromLocations(locations))
			{
				if (item2.Data is AssetBundleRequestOptions)
				{
					flag = flag && Caching.ClearAllCachedVersions((item2.Data as AssetBundleRequestOptions).BundleName);
				}
			}
		}
		return flag;
	...
	{
		handle.Completed += delegate(AsyncOperationHandle op)
		{
			Release(op);
		};
	}
	...
	{
		handle.Completed += delegate(AsyncOperationHandle<TObject> op)
		{
			Release(op);
		};
	}
	...
	public AsyncOperationHandle<bool> ClearDependencyCacheAsync(object key, bool autoReleaseHandle)
	{
		if (ShouldChainRequest)
		{
			AsyncOperationHandle<bool> asyncOperationHandle = ResourceManager.CreateChainOperation(ChainOperation, (AsyncOperationHandle op) => ClearDependencyCacheAsync(key, autoReleaseHandle));
			if (autoReleaseHandle)
			{
				AutoReleaseHandleOnCompletion(asyncOperationHandle);
			}
		...
		AsyncOperationHandle<bool> asyncOperationHandle2 = ResourceManager.CreateCompletedOperation(flag, flag ? string.Empty : "Unable to clear the cache.  AssetBundle's may still be loaded for the given key.");
		if (autoReleaseHandle)
		{
			AutoReleaseHandleOnCompletion(asyncOperationHandle2);
		}
	...
	public AsyncOperationHandle<bool> ClearDependencyCacheAsync(IList<IResourceLocation> locations, bool autoReleaseHandle)
	{
		if (ShouldChainRequest)
		{
			AsyncOperationHandle<bool> asyncOperationHandle = ResourceManager.CreateChainOperation(ChainOperation, (AsyncOperationHandle op) => ClearDependencyCacheAsync(locations, autoReleaseHandle));
			if (autoReleaseHandle)
			{
				AutoReleaseHandleOnCompletion(asyncOperationHandle);
			}
		...
		AsyncOperationHandle<bool> asyncOperationHandle2 = ResourceManager.CreateCompletedOperation(flag, flag ? string.Empty : "Unable to clear the cache.  AssetBundle's may still be loaded for the given key(s).");
		if (autoReleaseHandle)
		{
			AutoReleaseHandleOnCompletion(asyncOperationHandle2);
		}
	...
	public AsyncOperationHandle<bool> ClearDependencyCacheAsync(IEnumerable keys, bool autoReleaseHandle)
	{
		if (ShouldChainRequest)
		{
			AsyncOperationHandle<bool> asyncOperationHandle = ResourceManager.CreateChainOperation(ChainOperation, (AsyncOperationHandle op) => ClearDependencyCacheAsync(keys, autoReleaseHandle));
			if (autoReleaseHandle)
			{
				AutoReleaseHandleOnCompletion(asyncOperationHandle);
			}
	...
	{
		AsyncOperationHandle<SceneInstance> asyncOperationHandle = ResourceManager.ProvideScene(SceneProvider, location, loadMode, activateOnLoad, priority);
		if (trackHandle)
		{
	...
	{
		AsyncOperationHandle<SceneInstance> asyncOperationHandle = SceneProvider.ReleaseScene(ResourceManager, handle, unloadOptions);
		if (autoReleaseHandle)
		{
			AutoReleaseHandleOnCompletion(asyncOperationHandle, unloadSceneOpExcludeReleaseCallback: true);
		}
		return asyncOperationHandle;
	}
