...
public class MethodBodyReader
{
	...
	public void FinalizeILCodes(List<MethodInfo> transpilers, List<Label> endLabels, List<ExceptionBlock> endBlocks)
	{
		...
		result.Do(delegate(CodeInstruction codeInstruction2)
		{
			...
			{
				Emitter.MarkLabel(generator, label3);
			});
			...
			if (true)
			{
				...
				case OperandType.InlineSig:
					...
					if (!(obj is int))
					{
						throw new Exception(string.Concat("Wrong Emit argument type ", obj.GetType(), " in ", codeInstruction2));
					}
				...
				default:
				{
					...
					if (methodInfo == null)
					{
						throw new Exception(string.Concat("Unknown Emit argument type ", obj.GetType(), " in ", codeInstruction2));
					}
	...
	private void ReadOperand(ILInstruction instruction)
	{
		...
		case OperandType.InlineSwitch:
		{
			...
			{
				array[num4] = ilBytes.ReadInt32() + num3;
			}
		...
		{
			int num9 = ilBytes.ReadInt32();
			instruction.operand = num9;
			instruction.argument = (int)instruction.operand;
			break;
		}
		case OperandType.ShortInlineR:
		{
			float num7 = ilBytes.ReadSingle();
		...
		{
			double num6 = ilBytes.ReadDouble();
		...
		case OperandType.InlineSig:
		{
			...
			Debugger.Log(0, "TEST", "Signature = " + source.Select((byte b5) => $"0x{b5:x02}").Aggregate((string a, string text) => a + " " + text) + "\n");
			Debugger.Break();
			break;
		}
