...
public static class Emitter
{
	...
	public static void LogIL(ILGenerator il, OpCode opCode, object argument)
	{
		...
		{
			string text = FormatArgument(argument);
	...
	public static void LogLocalVariable(ILGenerator il, LocalBuilder variable)
	{
		...
		{
			int num = -1;
			LocalBuilder[] array = ((localsGetter != null) ? ((LocalBuilder[])localsGetter(il)) : null);
			num = ((array == null || array.Length == 0) ? ((int)localCountGetter(il)) : array.Length);
			string str = string.Format("{0}Local var {1}: {2}{3}", CodePos(il), num - 1, variable.LocalType.FullName, variable.IsPinned ? "(pinned)" : "");
	...
	public static void MarkLabel(ILGenerator il, Label label)
	{
		if (HarmonyInstance.DEBUG)
		{
			FileLog.LogBuffered(CodePos(il) + FormatArgument(label));
		}
		il.MarkLabel(label);
	}
	...
	public static void Emit(ILGenerator il, OpCode opcode)
	{
		if (HarmonyInstance.DEBUG)
		{
			FileLog.LogBuffered(CodePos(il) + opcode);
		}
		il.Emit(opcode);
	}
	...
	public static void EmitCall(ILGenerator il, OpCode opcode, MethodInfo methodInfo, Type[] optionalParameterTypes)
	{
		if (HarmonyInstance.DEBUG)
		{
			FileLog.LogBuffered($"{CodePos(il)}Call {opcode} {methodInfo} {optionalParameterTypes}");
		}
	...
	public static void EmitCalli(ILGenerator il, OpCode opcode, CallingConvention unmanagedCallConv, Type returnType, Type[] parameterTypes)
	{
		if (HarmonyInstance.DEBUG)
		{
			FileLog.LogBuffered($"{CodePos(il)}Calli {opcode} {unmanagedCallConv} {returnType} {parameterTypes}");
		}
	...
	public static void EmitCalli(ILGenerator il, OpCode opcode, CallingConventions callingConvention, Type returnType, Type[] parameterTypes, Type[] optionalParameterTypes)
	{
		if (HarmonyInstance.DEBUG)
		{
			FileLog.LogBuffered($"{CodePos(il)}Calli {opcode} {callingConvention} {returnType} {parameterTypes} {optionalParameterTypes}");
		}
