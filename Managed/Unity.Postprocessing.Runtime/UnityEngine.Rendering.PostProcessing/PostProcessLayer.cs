...
public sealed class PostProcessLayer : MonoBehaviour
{
	...
	[SerializeField]
	private AssetReference m_ResourcesReference;

	private PostProcessResources m_LoadedResource;
	...
	public void InitBundles()
	{
		...
		{
			m_LoadedResource = m_ResourcesReference.LoadAssetAsync<PostProcessResources>().WaitForCompletion();
		}
	...
	private void OnDisable()
	{
		...
		if (haveBundlesBeenInited)
		{
			...
			m_ResourcesReference.ReleaseAsset();
		}
		TextureLerper.instance.Discard();
		haveBundlesBeenInited = false;
		m_CurrentContext.resources = null;
	...
	public void BakeMSVOMap(CommandBuffer cmd, Camera camera, RenderTargetIdentifier destination, RenderTargetIdentifier? depthMap, bool invert, bool isMSAA = false)
	{
		...
		multiScaleVO.SetResources(m_LoadedResource);
	...
	{
		if (m_OldResources != m_LoadedResource || !RuntimeUtilities.isValidResources())
		{
			RuntimeUtilities.UpdateResources(m_LoadedResource);
			m_OldResources = m_LoadedResource;
		}
		m_IsRenderingInSceneView = context.camera.cameraType == CameraType.SceneView;
		context.isSceneView = m_IsRenderingInSceneView;
		context.resources = m_LoadedResource;
