...
public sealed class AmbientOcclusion : PostProcessEffectSettings
{
	...
	public override bool IsEnabledAndSupported(PostProcessRenderContext context)
	{
		...
		else if (mode.value == AmbientOcclusionMode.MultiScaleVolumetricObscurance)
		{
			...
			flag &= SystemInfo.supportsComputeShaders && !RuntimeUtilities.isAndroidOpenGL && SystemInfo.IsFormatSupported(GraphicsFormat.R32_SFloat, FormatUsage.GetPixels) && SystemInfo.IsFormatSupported(GraphicsFormat.R16_SFloat, FormatUsage.GetPixels) && SystemInfo.IsFormatSupported(GraphicsFormat.R8_UNorm, FormatUsage.GetPixels);
		}
		return flag;
	}
}
