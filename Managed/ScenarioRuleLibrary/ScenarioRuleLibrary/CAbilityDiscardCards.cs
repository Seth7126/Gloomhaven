...
internal class CAbilityDiscardCards : CAbilityTargeting
{
	public CAbilityDiscardCards()
		: base(EAbilityType.DiscardCards)
	{
	}
	...
	public override void Start(CActor targetActor, CActor filterActor, CActor controllingActor = null)
	{
		...
		if (base.AbilityFilter.HasTargetTypeFlag(CAbilityFilter.EFilterTargetType.Self, exclusive: true) && (targetActor.Type != CActor.EType.Player || ((CPlayerActor)targetActor).CharacterClass.HandAbilityCards.Count <= 0))
		{
		...
		if (base.ApplyToActor(actor))
		{
			...
			if (actor.Type == CActor.EType.Player && ((CPlayerActor)actor).CharacterClass.HandAbilityCards.Count > 0)
			{
				if (m_Strength == int.MaxValue)
				{
					...
					{
						cPlayerActor.CharacterClass.MoveAbilityCard(cPlayerActor.CharacterClass.HandAbilityCards[num], cPlayerActor.CharacterClass.HandAbilityCards, cPlayerActor.CharacterClass.DiscardedAbilityCards, "HandAbilityCards", "DiscardedAbilityCards");
					}
	...
	public CAbilityDiscardCards(CAbilityDiscardCards state, ReferenceDictionary references)
