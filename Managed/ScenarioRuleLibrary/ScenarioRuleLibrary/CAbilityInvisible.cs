...
public class CAbilityInvisible : CAbilityCondition
{
	public CAbilityInvisible(int duration, EConditionDecTrigger decTrigger)
		: base(EAbilityType.Invisible, duration, decTrigger)
	{
	}
	...
	public override bool ApplyToActor(CActor actor)
	{
		if (base.ApplyToActor(actor))
		{
			...
			if (miscAbilityData != null && miscAbilityData.AlreadyHasConditionDamageInstead.HasValue && actor.Tokens.HasKey(CCondition.EPositiveCondition.Invisible))
			{
			...
			{
				actor.ApplyCondition(base.TargetingActor, CCondition.EPositiveCondition.Invisible, base.Duration, base.DecrementTrigger);
	...
	public CAbilityInvisible()
	{
	}

	public CAbilityInvisible(CAbilityInvisible state, ReferenceDictionary references)
