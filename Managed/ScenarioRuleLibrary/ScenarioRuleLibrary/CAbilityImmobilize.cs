...
public class CAbilityImmobilize : CAbilityCondition
{
	public CAbilityImmobilize(int duration, EConditionDecTrigger decTrigger)
		: base(EAbilityType.Immobilize, duration, decTrigger)
	{
	}
	...
	public override bool ApplyToActor(CActor actor)
	{
		if (base.ApplyToActor(actor))
		{
			...
			if (miscAbilityData != null && miscAbilityData.AlreadyHasConditionDamageInstead.HasValue && actor.Tokens.HasKey(CCondition.ENegativeCondition.Immobilize))
			{
			...
			{
				actor.ApplyCondition(base.TargetingActor, CCondition.ENegativeCondition.Immobilize, base.Duration, base.DecrementTrigger);
	...
	public CAbilityImmobilize()
	{
	}

	public CAbilityImmobilize(CAbilityImmobilize state, ReferenceDictionary references)
