...
public sealed class DMDEmitMethodBuilderGenerator : DMDGenerator<DMDEmitMethodBuilderGenerator>
{
	...
	public static MethodBuilder GenerateMethodBuilder(DynamicMethodDefinition dmd, TypeBuilder typeBuilder)
	{
		...
		if (typeBuilder == null)
		{
			string environmentVariable = Environment.GetEnvironmentVariable("MONOMOD_DMD_DUMP");
			environmentVariable = ((!string.IsNullOrEmpty(environmentVariable)) ? Path.GetFullPath(environmentVariable) : null);
			bool flag = string.IsNullOrEmpty(environmentVariable) && _MBCanRunAndCollect;
			...
			}, flag ? ((AssemblyBuilderAccess)9) : AssemblyBuilderAccess.RunAndSave, environmentVariable);
			...
			typeBuilder = assemblyBuilder.DefineDynamicModule(assemblyBuilder.GetName().Name + ".dll", assemblyBuilder.GetName().Name + ".dll", dmd.Debug).DefineType($"DMD<{originalMethod?.GetID(null, null, withType: true, proxyMethod: false, simple: true)?.Replace('.', '_')}>?{dmd.GetHashCode()}", System.Reflection.TypeAttributes.Public | System.Reflection.TypeAttributes.Abstract | System.Reflection.TypeAttributes.Sealed);
		}
		Type[] array;
		Type[][] array2;
		Type[][] array3;
		...
		else
		{
			...
			{
				_DMDEmit.ResolveWithModifiers(definition.Parameters[j].ParameterType, out var type2, out var typeModReq, out var typeModOpt, modReq, modOpt);
				array[j + num2] = type2;
				array2[j + num2] = typeModReq;
				array3[j + num2] = typeModOpt;
			}
		}
		_DMDEmit.ResolveWithModifiers(definition.ReturnType, out var type3, out var typeModReq2, out var typeModOpt2);
		MethodBuilder methodBuilder = typeBuilder.DefineMethod((originalMethod?.Name ?? definition.Name).Replace('.', '_'), System.Reflection.MethodAttributes.Public | System.Reflection.MethodAttributes.Static | System.Reflection.MethodAttributes.HideBySig, CallingConventions.Standard, type3, typeModReq2, typeModOpt2, array, array2, array3);
