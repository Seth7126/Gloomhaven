...
public class AudioObject : RegisteredComponent
{
	...
	private AudioSource _audioSourceHapticVibration;

	private AudioSource _audioSourceHapticAudio;
	...
	public AudioSource HapticVibrationAudioSource => _audioSourceHapticVibration;

	public AudioSource HapticAudioAudioSource => _audioSourceHapticAudio;
	...
	{
		_ = primaryAudioSource != null;
		_ = secondaryAudioSource != null;
	}
	...
	{
		yield return Timekeeper.instance.WaitForSeconds(startToFadeTime);
		if (!_IsInactive)
		{
			Stop(fadeOutLength);
		}
	...
	{
		if (!_IsInactive && !IsPaused(returnTrueIfStillFadingOut: false))
		{
	...
	private void PlayHaptics(float delay)
	{
		if (audioItem.HapticVibrationEnabled || audioItem.HapticAudioEnabled)
		{
			if (delay > 0.1f)
			{
				Invoke("PlayHapticsDelayed", delay);
			}
			else
			{
				PlayHapticsDelayed();
	...
	private void PlayHapticsDelayed()
	{
	}

	private void StopHaptics()
	{
		HapticVibrationAudioSource.Stop();
		HapticAudioAudioSource.Stop();
	}
	...
	private void Update()
	{
		...
		_primarySourceWasPlayingLastUpdate = flag;
		_secondarySourceWasPlayingLastUpdate = flag2;
	...
	private void _StartFadeOutIfNecessary()
	{
		...
		{
			num2 = subItem.FadeOut;
		}
		else if (_stopClipAtTime > 0f)
		{
			num2 = 0.1f;
		}
		if (!isFadingOutOrScheduled && num2 > 0f && num > clipLength - num2)
		{
	...
	internal void _ApplyHapticVolume()
	{
	}

	internal void _ApplyHapticVibrationStrength()
	{
	}
