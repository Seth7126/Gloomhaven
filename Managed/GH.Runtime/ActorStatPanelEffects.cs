...
public class ActorStatPanelEffects : MonoBehaviour
{
	...
	[SerializeField]
	private Image _lowGrostEffect;

	[SerializeField]
	private Image[] _disableImagesForLowGraphic;
	...
	[SerializeField]
	private float _endedFlowValue = 1f;
	...
	private bool _useLowEffect
	{
		get
		{
			...
			{
				return PlatformLayer.Setting.SimplifiedUISettings.LowGostEffectActorPanel;
			}
			return false;
		}
	}
	...
	private void Initialize()
	{
		...
		else
		{
			...
			for (int m = 0; m < imgCount; m++)
			{
				if (imgComp[m] != null)
				{
					imgComp[m].material = null;
				}
			}
			for (int n = 0; n < _disableImagesForLowGraphic.Length; n++)
			{
				_disableImagesForLowGraphic[n].enabled = false;
			}
		}
		RestoreCard();
	}
	...
	{
		RestoreCard();
		ToggleAdditiveEffect(active);
	}

	private void ToggleAdditiveEffect(bool active)
	{
		if (active)
		{
	...
	private void GhostOutOn(bool ghostAnim)
	{
		if (_useLowEffect)
		{
			_lowGrostEffect.enabled = true;
		}
		else if (Choreographer.s_Choreographer != null)
		{
	...
	private IEnumerator BurnCardTimeline(bool burnAnim)
	{
		...
		for (int i = 0; i < imgCount; i++)
		{
			if (imgComp[i] != null && imgComp[i].material.shader.name == "AbilityCard_Shd")
			{
			...
			{
				Debug.LogError("imgComp missing or GUI shader not meant for current script!!");
			}
		}
	...
	private IEnumerator GhostOutOnTimeline(bool ghostAnim)
	{
		...
		for (int i = 0; i < imgCount; i++)
		{
			if (imgComp[i] != null)
			{
				if (imgComp[i].material.HasProperty(_burn))
				{
					imgComp[i].material.SetFloat(_burn, mc_Burn);
				}
				else
				{
					Debug.LogErrorFormat("Missing _Burn property in {0} shader" + imgComp[i].name);
				}
				if (imgComp[i].material.HasProperty(_burnColourTint))
				{
					imgComp[i].material.SetColor(_burnColourTint, mc_Burn_ColourTint);
				}
				else
				{
					Debug.LogErrorFormat("Missing _Burn_ColourTint property in {0} shader" + imgComp[i].name);
				}
				if (imgComp[i].material.HasProperty(_flowOffset))
				{
					imgComp[i].material.SetFloat(_flowOffset, mc_Flow_Offset);
				}
				else
				{
					Debug.LogErrorFormat("Missing _Flow_Offset property in {0} shader" + imgComp[i].name);
				}
				if (imgComp[i].material.HasProperty(_flowSpeed))
				{
					imgComp[i].material.SetFloat(_flowSpeed, mc_Flow_Speed);
				}
				else
				{
					Debug.LogErrorFormat("Missing _Flow_Speed property in {0} shader" + imgComp[i].name);
				}
				if (imgComp[i].material.HasProperty(_animNoiseMask))
				{
					imgComp[i].material.SetTextureScale(_animNoiseMask, new Vector2(mc_AnimNoise_Mask_tile, mc_AnimNoise_Mask_tile));
				}
				else
				{
					Debug.LogErrorFormat("Missing _AnimNoise_Mask property in {0} shader" + imgComp[i].name);
				}
				if (imgComp[i].material.HasProperty(_dissolveVerticalGradient))
				{
					imgComp[i].material.SetFloat(_dissolveVerticalGradient, mc_Dissolve_VerticalGradient);
				}
				else
				{
					Debug.LogErrorFormat("Missing _Dissolve_VerticalGradient property in {0} shader" + imgComp[i].name);
				}
			}
			else
			{
				Debug.LogError("imgComp missing");
			}
		}
		fgFx.material.SetColor(_tintColor, fx_Overlay_color);
		fgFx.material.SetTexture(_particleTexture, fx_Overlay_texture);
		...
		else
		{
			for (int l = 0; l < imgCount; l++)
			{
				...
				{
					imgComp[l].material.SetFloat(_greyOut, 1f);
					imgComp[l].material.SetFloat(_flow, _endedFlowValue);
					imgComp[l].material.SetFloat(_dissolve, mc_Dissolve);
				}
			}
			Debug.Log("right");
