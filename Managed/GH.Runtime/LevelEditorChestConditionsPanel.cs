...
public class LevelEditorChestConditionsPanel : MonoBehaviour
{
	...
	private CObjectChest m_ChestShowingFor;

	private List<LevelEditorClassListItem> m_NegativeConditionItems;

	private List<string> m_AllPossibleConditions;

	public void DisplayConditionsForChest(CObjectChest chestToShow)
	{
		ListTitle.text = "Chest Conditions:";
		m_ChestShowingFor = chestToShow;
		FillList();
	}

	public void AddNegativeConditionForChest(CCondition.ENegativeCondition conditionToAdd)
	{
	...
	{
		m_ChestShowingFor.Conditions.RemoveAll((CCondition.ENegativeCondition c) => c.ToString() == conditionItemToRemove.ClassName);
	...
	{
		ClearList();
		foreach (CCondition.ENegativeCondition condition in m_ChestShowingFor.Conditions)
		{
			AddNegativeConditionForChest(condition);
		}
		RefreshAddDropDown();
	}
	...
	public void AddConditionPressed()
	{
		...
		if (m_AllPossibleConditions != null && dropDownIndexToUse >= 0 && dropDownIndexToUse < m_AllPossibleConditions.Count)
		{
			...
			if (eNegativeCondition != CCondition.ENegativeCondition.NA && !m_ChestShowingFor.Conditions.Contains(eNegativeCondition))
			{
				m_ChestShowingFor.Conditions.Add(eNegativeCondition);
				FillList();
			}
		}
	}
