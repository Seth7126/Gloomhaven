...
public class DistributeAttackModifierProcess : DistributeRewardProcess
{
	private class DistributeModifierService : DistributeRewardService
	{
		public DistributeModifierService(List<CMapCharacter> characters, Reward reward)
		...
		{
			return string.Format(LocalizationManager.GetTranslation("GUI_DISTRIBUTE_ATTACK_MODIFIER"), string.Join(", ", base.Reward.Modifiers.Select((KeyValuePair<string, int> it) => $"x{it.Value} {UIInfoTools.Instance.GetAttackModifierText(ScenarioRuleClient.SRLYML.AttackModifiers.First((AttackModifierYMLData s) => s.Name == it.Key))}")));
		}
		...
		public override bool CanAddPointsTo(IDistributePointsActor actor)
		{
			...
			{
				return actors.Exists((DistributeMapActor it) => GetAssignedPoints(it) > 0 && it != actor);
			}
			return true;
		}
		...
		public override void AddPoint(IDistributePointsActor actor)
		{
			if (base.AvailablePoints > 0)
			{
				...
				if (FFSNetwork.IsHost)
				{
					...
					Synchronizer.SendGameAction(GameActionType.DistributeUIAddPoint, ActionPhaseType.NONE, validateOnServerBeforeExecuting: false, disableAutoReplication: false, actorID, 3);
				}
				return;
			}
			...
			if (base.AvailablePoints > 0)
			{
				...
				if (FFSNetwork.IsHost)
				{
					...
					Synchronizer.SendGameAction(GameActionType.DistributeUIAddPoint, ActionPhaseType.NONE, validateOnServerBeforeExecuting: false, disableAutoReplication: false, actorID2, 3);
				}
			}
		}
	...
	private DistributeModifierService m_Service;

	public override EDistributeRewardProcessType GetRewardType()
	{
		return EDistributeRewardProcessType.DistributeModifiers;
	}
	...
	public override ICallbackPromise Process(List<Reward> rewards)
	{
		...
		{
			callbackPromise = callbackPromise.Then(() => processUI.Distribute(m_Service = new DistributeModifierService(selectedCharacters, reward), GetRewardType(), "GUI_CONFIRM", "GUI_SELECT")).Then(delegate
			{
	...
	{
		if (reward.Type == ETreasureType.AddModifiers)
		{
