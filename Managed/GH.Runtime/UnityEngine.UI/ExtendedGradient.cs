...
[AddComponentMenu("UI/Effects/ExtendedGradient")]
public class ExtendedGradient : BaseMeshEffect
{
	public enum Type
	{
		Horizontal,
	...
	[SerializeField]
	[ConditionalField("_gradientType", Type.Radial, true)]
	private bool _useDefaultImageColor;
	...
	public override void ModifyMesh(VertexHelper helper)
	{
		...
		case Type.Vertical:
		{
			...
			float num11 = bounds3.xMin;
			float num12 = bounds3.width;
			Func<UIVertex, float> func = (UIVertex uIVertex) => uIVertex.position.x;
			if (GradientType == Type.Vertical)
			{
				num11 = bounds3.yMin;
				num12 = bounds3.height;
				func = (UIVertex uIVertex) => uIVertex.position.y;
			}
			float num13 = ((num12 == 0f) ? 0f : (1f / num12 / Zoom));
			float num14 = (1f - 1f / Zoom) * 0.5f;
			float num15 = Offset * (1f - num14) - num14;
			if (ModifyVertices)
			{
				SplitTrianglesAtGradientStops(list, bounds3, num14, helper);
			}
			UIVertex vertex4 = default(UIVertex);
			for (int num16 = 0; num16 < helper.currentVertCount; num16++)
			{
				helper.PopulateUIVertex(ref vertex4, num16);
				vertex4.color = BlendColor(vertex4.color, EffectGradient.Evaluate((func(vertex4) - num11) * num13 - num15));
				helper.SetUIVertex(vertex4, num16);
			}
			break;
		}
		case Type.Diamond:
		{
			...
			float num9 = ((bounds2.height == 0f) ? 0f : (1f / bounds2.height / Zoom));
			float num10 = bounds2.center.y / 2f;
			Vector3 vector = (Vector3.right + Vector3.up) * num10 + Vector3.forward * list[0].position.z;
			if (ModifyVertices)
			{
				helper.Clear();
				for (int l = 0; l < count; l++)
				{
					helper.AddVert(list[l]);
				}
				...
				{
					helper.AddTriangle(m - 1, m, count);
				}
				helper.AddTriangle(0, count - 1, count);
			}
			UIVertex vertex3 = default(UIVertex);
			for (int n = 0; n < helper.currentVertCount; n++)
			{
				helper.PopulateUIVertex(ref vertex3, n);
				vertex3.color = BlendColor(vertex3.color, EffectGradient.Evaluate(Vector3.Distance(vertex3.position, vector) * num9 - Offset));
				helper.SetUIVertex(vertex3, n);
			}
			break;
		}
		case Type.Radial:
		{
			...
			if (ModifyVertices)
			{
				...
				helper.Clear();
				float num4 = bounds.width / 2f;
				float num5 = bounds.height / 2f;
				...
				for (int i = 0; i < num3; i++)
				{
					...
					float num6 = (float)i * 360f / (float)num3;
					float num7 = Mathf.Cos(MathF.PI / 180f * num6);
					float num8 = Mathf.Sin(MathF.PI / 180f * num6);
					v2.position = Vector3.right * num7 * num4 + Vector3.up * num8 * num5 + Vector3.forward * list[0].position.z;
				...
				{
					helper.AddTriangle(j - 1, j, num3);
				}
				helper.AddTriangle(0, num3 - 1, num3);
			}
			UIVertex vertex2 = default(UIVertex);
			for (int k = 0; k < helper.currentVertCount; k++)
			{
				helper.PopulateUIVertex(ref vertex2, k);
				vertex2.color = BlendColor(vertex2.color, EffectGradient.Evaluate(Mathf.Sqrt(Mathf.Pow(Mathf.Abs(vertex2.position.x - bounds.center.x) * num, 2f) + Mathf.Pow(Mathf.Abs(vertex2.position.y - bounds.center.y) * num2, 2f)) * 2f - Offset));
				helper.SetUIVertex(vertex2, k);
			}
			break;
		}
		}
