...
internal static class RenderEvents
{
	...
	private static bool UpdateTextCoreSettings(RenderChain renderChain, VisualElement ve)
	{
		...
		if ((!NeedsColorID(ve) || textCoreSettingsForElement.faceColor == Color.white) && textCoreSettingsForElement.outlineWidth == 0f && textCoreSettingsForElement.underlayOffset == Vector2.zero && textCoreSettingsForElement.underlaySoftness == 0f && !flag)
		{
			ve.renderChainData.textCoreSettingsID = UIRVEShaderInfoAllocator.defaultTextCoreSettings;
			return true;
		}
