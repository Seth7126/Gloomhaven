...
public class VisualElement : Focusable, IStylePropertyAnimations, ITransform, ITransitionAnimations, IExperimentalFeatures, IVisualElementScheduler, IResolvedStyle
{
	...
	private Matrix4x4 pivotedMatrixWithLayout
	{
		get
		{
			Vector3 vector = ResolveTransformOrigin();
			Matrix4x4 lhs = Matrix4x4.TRS(positionWithLayout + vector, ResolveRotation(), ResolveScale());
			TranslateMatrix34InPlace(ref lhs, -vector);
			return lhs;
		}
	}
	...
	internal Vector3 ComputeGlobalScale()
	{
		...
		{
			value.Scale(visualElement.resolvedStyle.scale.value);
		}
		return value;
	}
	...
	internal void UpdateWorldTransform()
	{
		...
		if (hierarchy.parent != null)
		{
			...
			{
				Matrix4x4 rhs = pivotedMatrixWithLayout;
				MultiplyMatrix34(ref hierarchy.parent.worldTransformRef, ref rhs, out m_WorldTransformCache);
			}
		}
		else
		{
	...
	private void TransformAlignedRectToParentSpace(ref Rect rect)
	{
		...
		Matrix4x4 matrix = pivotedMatrixWithLayout;
