...
public class ScrollView : VisualElement
{
	...
	internal const float ScrollThresholdSquared = 25f;
	...
	private float scrollableWidth => contentContainer.boundingBox.width - contentViewport.layout.width;

	private float scrollableHeight => contentContainer.boundingBox.height - contentViewport.layout.height;
	...
	private void OnPointerMove(PointerMoveEvent evt)
	{
		...
		if (m_StartedMoving || !((vector - m_PointerStartPosition).sqrMagnitude < 25f))
		{
			m_StartedMoving = true;
			if (ComputeTouchScrolling(evt.position))
			{
	...
	internal bool ComputeTouchScrolling(Vector2 position)
	{
		...
		{
			lhs = m_StartPosition - (new Vector2(position.x, position.y) - m_PointerStartPosition);
			lhs = Vector2.Max(lhs, m_LowBounds);
			lhs = Vector2.Min(lhs, m_HighBounds);
		}
		else if (touchScrollBehavior == TouchScrollBehavior.Elastic)
		{
			Vector2 vector = new Vector2(position.x, position.y) - m_PointerStartPosition;
		...
		if (hasInertia)
		{
			if (lhs == m_LowBounds || lhs == m_HighBounds)
			{
				m_Velocity = Vector2.zero;
				scrollOffset = lhs;
				return false;
			...
			Vector2 b = (lhs - scrollOffset) / unscaledDeltaTime;
			m_Velocity = Vector2.Lerp(m_Velocity, b, unscaledDeltaTime * 10f);
		}
		bool result = scrollOffset != lhs;
		scrollOffset = lhs;
		return result;
	}
	...
	private void OnScrollWheel(WheelEvent evt)
	{
		...
		if (flag3)
		{
			...
			horizontalScroller.value += num * ((horizontalScroller.lowValue < horizontalScroller.highValue) ? 1f : (-1f)) * m_SingleLineHeight;
			if (horizontalScroller.value != value2)
			{
				evt.StopPropagation();
				flag = true;
			}
