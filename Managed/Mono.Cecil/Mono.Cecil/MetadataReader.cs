...
internal sealed class MetadataReader : ByteBuffer
{
	...
	public Collection<TypeReference> ReadGenericConstraints(GenericParameter generic_parameter)
	{
		...
		Collection<TypeReference> collection = new Collection<TypeReference>(mapping.Count);
	...
	private ConstantDebugInformation ReadLocalConstant(uint rid)
	{
		...
		{
			value = (typeReference.IsTypeOf("System", "DateTime") ? ((object)new DateTime(signatureReader.ReadInt64())) : ((typeReference.etype != ElementType.Object && typeReference.etype != ElementType.None && typeReference.etype != ElementType.Class) ? signatureReader.ReadConstantSignature(typeReference.etype) : null));
		}
		else
		{
		...
		for (int i = 0; i < value.Length; i++)
		{
			...
			else if (value[i].Col1 == EmbeddedSourceDebugInformation.KindIdentifier)
			{
				...
				else if (num2 > 0)
				{
					...
					MemoryStream target = new MemoryStream(content);
					using (DeflateStream self = new DeflateStream(stream, CompressionMode.Decompress, leaveOpen: true))
					{
						self.CopyTo(target);
					}
