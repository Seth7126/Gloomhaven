...
internal class ManateeSearch : IJsonSearch, IJsonSerializable
{
	...
	public JsonValue ToJson(JsonSerializer serializer)
	{
		...
		Types.Serialize(jsonObject, serializer, "types");
		Limit.Serialize(jsonObject, serializer, "boards_limit");
		Limit.Serialize(jsonObject, serializer, "cards_limit");
		Limit.Serialize(jsonObject, serializer, "organizations_limit");
		Limit.Serialize(jsonObject, serializer, "members_limit");
		Partial.Serialize(jsonObject, serializer, "partial");
		if (Context != null)
		{
			TryAddContext<IJsonCard>(jsonObject, "idCards");
			TryAddContext<IJsonBoard>(jsonObject, "idBoards");
			TryAddContext<IJsonOrganization>(jsonObject, "idOrganizations");
		}
		return jsonObject;
	}

	private void TryAddContext<T>(JsonObject json, string key) where T : IJsonCacheable
	{
		string text = string.Join(",", from o in Context.OfType<T>()
			select o.Id);
