...
public sealed class Texture2D : Texture
{
	...
	public unsafe NativeArray<T> GetPixelData<T>(int mipLevel) where T : struct
	{
		...
		int pixelDataOffset = GetPixelDataOffset(mipLevel);
		int pixelDataSize = GetPixelDataSize(mipLevel);
		int num = UnsafeUtility.SizeOf<T>();
		IntPtr intPtr = new IntPtr(GetWritableImageData(0).ToInt64() + pixelDataOffset);
