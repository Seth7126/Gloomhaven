...
public sealed class Texture2DArray : Texture
{
	...
	public unsafe NativeArray<T> GetPixelData<T>(int mipLevel, int element) where T : struct
	{
		...
		int pixelDataOffset = GetPixelDataOffset(base.mipmapCount, element);
		int pixelDataOffset2 = GetPixelDataOffset(mipLevel, element);
		int pixelDataSize = GetPixelDataSize(mipLevel, element);
		int num = UnsafeUtility.SizeOf<T>();
		IntPtr intPtr = new IntPtr(GetImageDataPointer().ToInt64() + (pixelDataOffset * element + pixelDataOffset2));
