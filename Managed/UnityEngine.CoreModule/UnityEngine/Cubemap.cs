...
public sealed class Cubemap : Texture
{
	...
	public unsafe NativeArray<T> GetPixelData<T>(int mipLevel, CubemapFace face) where T : struct
	{
		...
		int pixelDataOffset = GetPixelDataOffset(base.mipmapCount, (int)face);
		int pixelDataOffset2 = GetPixelDataOffset(mipLevel, (int)face);
		int pixelDataSize = GetPixelDataSize(mipLevel, (int)face);
		int num = UnsafeUtility.SizeOf<T>();
		IntPtr intPtr = new IntPtr(GetWritableImageData(0).ToInt64() + (pixelDataOffset * (int)face + pixelDataOffset2));
