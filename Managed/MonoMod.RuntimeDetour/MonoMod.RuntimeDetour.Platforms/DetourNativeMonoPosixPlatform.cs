...
public sealed class DetourNativeMonoPosixPlatform : IDetourNativePlatform
{
	[Flags]
	...
	{
		Inner = inner;
		_Pagesize = sysconf(SysconfName._SC_PAGESIZE, (Errno)0);
	}

	private static string GetLastError(string name)
	{
		int num = _GetLastError();
		if (ToErrno(num, out var rval) == 0)
		{
			return $"{name} returned {rval}";
		}
		return $"{name} returned 0x${num:X8}";
	}
	...
	private void SetMemPerms(IntPtr start, ulong len, MmapProts prot)
	{
		...
		if (mprotect((IntPtr)num, (ulong)(num2 - num), prot) != 0)
		{
