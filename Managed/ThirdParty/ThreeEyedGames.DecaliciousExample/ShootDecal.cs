...
public class ShootDecal : MonoBehaviour
{
	...
	private void Update()
	{
		...
		if (Physics.Raycast(GetComponent<Camera>().ViewportPointToRay(Vector3.one * 0.5f), out var hitInfo))
		{
			Transform transform = Object.Instantiate(DecalPrefab, hitInfo.collider.transform, worldPositionStays: true).transform;
