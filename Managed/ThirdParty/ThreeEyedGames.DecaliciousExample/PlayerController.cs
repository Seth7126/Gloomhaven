...
public class PlayerController : MonoBehaviour
{
	...
	private void Update()
	{
		...
		if (Input.GetKeyDown(KeyCode.E) && Physics.Raycast(Camera.main.ViewportPointToRay(Vector3.one * 0.5f), out var hitInfo))
		{
			bool flag = false;
			if (Vector3.SqrMagnitude(base.transform.position - hitInfo.point) < 2.25f && hitInfo.collider.GetComponent<IInteract>() != null)
			{
				hitInfo.collider.GetComponent<IInteract>().Interact();
				flag = true;
			}
			AudioSource.PlayClipAtPoint(flag ? InteractSuccess : InteractError, base.transform.position + base.transform.forward * 0.5f);
		}
