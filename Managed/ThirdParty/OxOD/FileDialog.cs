...
public class FileDialog : MonoBehaviour
{
	...
	public void GoTo(string newPath)
	{
		...
		{
			newPath = newPath.Replace("%" + value.ToString() + "%", Environment.GetFolderPath(value));
		}
	...
	public void SelectFile(string file, bool withoutConfirm = false)
	{
			...
			if (allowedExtensions != null)
			{
				...
				for (int num = 0; num < array.Length; num++)
				{
					...
					else
					{
						...
						component3.data = array[num].FullName;
						component3.size.text = GetFileSizeText(array[num].Length);
						component3.elementName.text = array[num].Name;
			...
			for (int num2 = 0; num2 < files.Length; num2++)
			{
				if (maxSize > 0)
				{
					if (files[num2].Length < maxSize)
					{
						...
						component4.data = files[num2].FullName;
						component4.size.text = GetFileSizeText(files[num2].Length);
						component4.elementName.text = files[num2].Name;
				...
				else
				{
					...
					component5.data = files[num2].FullName;
					component5.size.text = GetFileSizeText(files[num2].Length);
					component5.elementName.text = files[num2].Name;
