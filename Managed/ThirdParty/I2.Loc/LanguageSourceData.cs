...
public class LanguageSourceData
{
	...
	[NonSerialized]
	public Dictionary<string, TermData> mDictionary = new Dictionary<string, TermData>(StringComparer.OrdinalIgnoreCase);
	...
	public string Export_CSV2(string[] languages, char Separator = ',', bool specializationsAsRows = true)
	{
		StringBuilder stringBuilder = new StringBuilder();
		int num = 0;
		...
		foreach (LanguageData mLanguage in mLanguages)
		{
			if (languages.Contains(GoogleLanguages.GetCodedLanguage(mLanguage.Name, mLanguage.Code)))
			{
				num++;
				stringBuilder.Append(Separator);
				if (!mLanguage.IsEnabled())
		...
		foreach (TermData mTerm in mTerms)
		{
			string term = mTerm.Term;
			if (specializationsAsRows)
			{
				foreach (string allSpecialization in mTerm.GetAllSpecializations())
				{
					AppendTermShort(stringBuilder, num, term, mTerm, allSpecialization, Separator);
				}
			}
			else
			{
				AppendTermShort(stringBuilder, num, term, mTerm, null, Separator);
			}
		}
		return stringBuilder.ToString();
	}

	private static void AppendTermShort(StringBuilder Builder, int nLanguages, string Term, TermData termData, string specialization, char Separator)
	{
	...
	public string Import_CSV(string Category, string CSVstring, eSpreadsheetUpdateMode UpdateMode = eSpreadsheetUpdateMode.Replace, char Separator = ',')
	{
		...
		foreach (string[] item in list)
		{
			for (int i = 0; i < item.Length; i++)
			{
				...
				{
					item[i] = item[i].Substring(1, item[i].Length - 2);
				}
			}
		}
		return Import_CSV(Category, list, UpdateMode);
	}
	...
	public bool TryGetTranslation(string term, out string Translation, string overrideLanguage = null, string overrideSpecialization = null, bool skipDisabled = false, bool allowCategoryMistmatch = false)
	{
		...
		if (languageIndex >= 0 && (!skipDisabled || mLanguages[languageIndex].IsEnabled()))
		{
			...
			{
				bool flag = TryGetFallbackTranslation(termData, out Translation, languageIndex, overrideSpecialization, skipDisabled);
				if (flag)
				{
					Translation = "<color=\"red\">" + Translation + "</color>";
					Debug.LogError("Missing " + (overrideLanguage ?? LocalizationManager.CurrentLanguage) + " translation for " + term + " loc key");
				}
				return flag;
			}
