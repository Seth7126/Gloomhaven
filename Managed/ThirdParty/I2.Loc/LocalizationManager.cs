...
public static class LocalizationManager
{
	...
	public static string[] GlobalSources = new string[3] { "I2Languages", "I2LanguagesConsoles", "I2LanguagesEULA" };
	...
	{
		Debug.Log("[LOCALIZATION]: Attempting to change to LanguageName: " + LanguageName + " LanguageCode: " + LanguageCode + " RememberLanguage:" + RememberLanguage + " \n" + Environment.StackTrace);
		if (mCurrentLanguage != LanguageName || mLanguageCode != LanguageCode || Force)
		{
			if (RememberLanguage)
			{
				Debug.Log("[LocalizationManager_Language] about to save to PersistentStorage key: I2 Language => value: " + LanguageName + ". ");
	...
	private static void SelectStartupLanguage()
	{
		...
		Debug.Log("[LocalizationManager_Language] About to load language settings string (I2 Language) from persistent storage...");
		string setting_String = PersistentStorage.GetSetting_String("I2 Language", string.Empty);
		Debug.Log("[LocalizationManager_Language] SelectStartupLanguage() SavedLanguage read from persistent storage is " + setting_String + ".");
		string currentDeviceLanguage = GetCurrentDeviceLanguage();
		Debug.Log("[LocalizationManager_Language] SelectStartupLanguage() SysLanguage is " + currentDeviceLanguage + ".");
		Debug.Log("Language Sources Count " + Sources.Count);
		foreach (LanguageSourceData source in Sources)
		{
			Debug.Log("Source: IgnoreDeviceLanguage:" + source.IgnoreDeviceLanguage);
			foreach (LanguageData mLanguage in source.mLanguages)
			{
				Debug.Log("Language Name: " + mLanguage.Name + ", Code: " + mLanguage.Code + ", Enabled: " + mLanguage.IsEnabled());
			}
		}
		if (!string.IsNullOrEmpty(setting_String) && HasLanguage(setting_String, AllowDiscartingRegion: true, Initialize: false))
		{
			Debug.Log("[LocalizationManager_Language] SelectStartupLanguage() going to setup SavedLanguage " + setting_String + ".");
		...
		Debug.Log("[LocalizationManager_Language] SelectStartupLanguage() tried to validate SysLnaguage (" + currentDeviceLanguage + ") into ValidLanguage (" + supportedLanguage + ").");
		if (!string.IsNullOrEmpty(supportedLanguage))
		{
			Debug.Log("[LocalizationManager_Language] SelectStartupLanguage() going to setup SavedLanguage as " + supportedLanguage + ".");
		...
		for (int count = Sources.Count; i < count; i++)
		{
			...
			for (int j = 0; j < Sources[i].mLanguages.Count; j++)
			{
				...
				{
					string name = Sources[i].mLanguages[j].Name;
					Debug.Log("[LocalizationManager_Language] SelectStartupLanguage() setting first available language (" + name + ") as default.");
					SetLanguageAndCode(name, Sources[i].mLanguages[j].Code, RememberLanguage: false);
					return;
				}
			}
		}
	...
	public static bool HasLanguage(string Language, bool AllowDiscartingRegion = true, bool Initialize = true, bool SkipDisabled = true)
	{
		...
		for (int count = Sources.Count; i < count; i++)
		{
			...
			{
				Debug.Log("[LocalizationManager_Language] HasLanguage(" + Language + ") returned true.");
				return true;
			}
		}
		if (AllowDiscartingRegion)
		{
			...
			for (int count2 = Sources.Count; j < count2; j++)
			{
				...
				{
					Debug.Log("[LocalizationManager_Language] HasLanguage(" + Language + ") returned true with discarded region.");
					return true;
				}
			}
		}
		Debug.LogError("[LocalizationManager_Language] HasLanguage(" + Language + ") returned false!");
		return false;
	}

	public static string GetSupportedLanguage(string Language, bool ignoreDisabled = false)
	{
		Debug.Log("[LocalizationManager_Language] GetSupportedLanguage (" + Language + ") was called...");
		string languageCode = GoogleLanguages.GetLanguageCode(Language);
		Debug.Log("[LocalizationManager_Language] GetSupportedLanguage (" + Language + ") GoogleLanguages.GetLanguageCode returned " + languageCode + ".");
		if (!string.IsNullOrEmpty(languageCode))
		{
			...
			for (int count = Sources.Count; i < count; i++)
			{
				...
				{
					string name = Sources[i].mLanguages[languageIndexFromCode].Name;
					Debug.Log("[LocalizationManager_Language] GetSupportedLanguage (" + Language + ") returned " + name);
					return name;
				}
			}
			int j = 0;
			for (int count2 = Sources.Count; j < count2; j++)
			{
				...
				{
					string name2 = Sources[j].mLanguages[languageIndexFromCode2].Name;
					Debug.Log("[LocalizationManager_Language] GetSupportedLanguage (" + Language + ") returned " + name2);
					return name2;
				}
			}
		}
		...
		for (int count3 = Sources.Count; k < count3; k++)
		{
			...
			if (languageIndex >= 0)
			{
				string name3 = Sources[k].mLanguages[languageIndex].Name;
				Debug.Log("[LocalizationManager_Language] GetSupportedLanguage (" + Language + ") returned " + name3);
				return name3;
			}
		}
		int l = 0;
		for (int count4 = Sources.Count; l < count4; l++)
		{
			...
			if (languageIndex2 >= 0)
			{
				string name4 = Sources[l].mLanguages[languageIndex2].Name;
				Debug.Log("[LocalizationManager_Language] GetSupportedLanguage (" + Language + ") returned " + name4);
				return name4;
			}
		}
		Debug.LogWarning("[LocalizationManager_Language] GetSupportedLanguage (" + Language + ") returned Empty String!!!");
		return string.Empty;
	}
	...
	public static string GetLanguageCode(string Language)
	{
		...
		for (int count = Sources.Count; i < count; i++)
		{
			...
			if (languageIndex >= 0)
			{
				string code = Sources[i].mLanguages[languageIndex].Code;
				Debug.Log("[LocalizationManager_Language] GetLanguageCode(" + Language + ") returned " + code);
				return code;
			}
		}
		Debug.LogWarning("[LocalizationManager_Language] GetLanguageCode(" + Language + ") returned string.Empty.");
		return string.Empty;
	}
	...
	public static string GetLanguageFromCode(string Code, bool exactMatch = true)
	{
		...
		for (int count = Sources.Count; i < count; i++)
		{
			...
			{
				string name = Sources[i].mLanguages[languageIndexFromCode].Name;
				Debug.Log("[LocalizationManager_Language] GetLanguageFromCode(" + Code + ") returned " + name);
				return name;
			}
		}
		Debug.LogWarning("[LocalizationManager_Language] GetLanguageFromCode(" + Code + ") returned string.Empty");
		return string.Empty;
	}
	...
	public static List<string> GetAllLanguages(bool SkipDisabled = true)
	{
		...
		Debug.Log("[LocalizationManager] GetAllLanguages() returned list containing: " + text);
		return Languages;
	}
