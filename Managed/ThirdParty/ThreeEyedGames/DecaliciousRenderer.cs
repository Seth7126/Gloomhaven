...
public class DecaliciousRenderer : MonoBehaviour
{
	...
	private void OnPreRender()
	{
		...
		SortedDictionary<int, Dictionary<Material, HashSet<Decal>>>.Enumerator enumerator = _deferredDecals.GetEnumerator();
	...
	private void DrawLimitToGameObjects(Camera cam)
	{
		...
		_bufferLimitTo.ClearRenderTarget(clearDepth: false, clearColor: true, Color.black);
		...
		foreach (GameObject limitToGameObject in _limitToGameObjects)
		{
			...
			foreach (MeshRenderer limitToMeshRenderer in _limitToMeshRenderers)
			{
				...
				if (_meshFilterComponent.Count == 1)
				{
					...
					if (limitToMeshRenderer.isPartOfStaticBatch)
					{
						_bufferLimitTo.DrawMesh(meshFilter.sharedMesh, Matrix4x4.identity, _materialLimitToGameObjects, limitToMeshRenderer.subMeshStartIndex);
					}
					else
					{
						_bufferLimitTo.DrawMesh(meshFilter.sharedMesh, limitToMeshRenderer.transform.localToWorldMatrix, _materialLimitToGameObjects, limitToMeshRenderer.subMeshStartIndex);
					}
				}
			}
	...
	private void DrawDeferredDecals_Albedo(Camera cam)
	{
		...
		while (enumerator.MoveNext())
		{
			Dictionary<Material, HashSet<Decal>>.Enumerator enumerator2 = enumerator.Current.Value.GetEnumerator();
			while (enumerator2.MoveNext())
			{
				Material key = enumerator2.Current.Key;
				HashSet<Decal> value = enumerator2.Current.Value;
				_ = value.Count;
				int num = 0;
				HashSet<Decal>.Enumerator enumerator3 = value.GetEnumerator();
				while (enumerator3.MoveNext())
				{
					Decal current = enumerator3.Current;
					...
					if (UseInstancing && !current.UseLightProbes)
					{
						...
						_limitToValues[num] = (current.LimitTo ? ((float)current.LimitTo.GetInstanceID()) : 0f);
						num++;
						if (num == 1023)
						{
					...
					else
					{
						...
						_directBlock.SetFloat("_LimitTo", current.LimitTo ? ((float)current.LimitTo.GetInstanceID()) : 0f);
						if (current.UseLightProbes)
						{
							LightProbes.GetInterpolatedProbe(current.transform.position, current.GetComponent<MeshRenderer>(), out var probe);
			...
			while (enumerator2.MoveNext())
			{
				...
				while (enumerator3.MoveNext())
				{
					...
					if (current.HighQualityBlending)
					{
						...
						_instancedBlock.SetFloat("_LimitTo", current.LimitTo ? ((float)current.LimitTo.GetInstanceID()) : 0f);
					...
					else if (UseInstancing)
					{
						...
						_limitToValues[num3] = (current.LimitTo ? ((float)current.LimitTo.GetInstanceID()) : 0f);
						num3++;
						if (num3 == 1023)
						{
							_bufferDeferred.Blit(BuiltinRenderTextureType.GBuffer1, num);
	...
	private void DrawUnlitDecals(Camera cam)
	{
		...
		SortedDictionary<int, Dictionary<Material, HashSet<Decal>>>.Enumerator enumerator = _unlitDecals.GetEnumerator();
		while (enumerator.MoveNext())
		{
			Dictionary<Material, HashSet<Decal>>.Enumerator enumerator2 = enumerator.Current.Value.GetEnumerator();
			while (enumerator2.MoveNext())
	...
	protected void AddDeferred(Decal decal)
	{
		...
		Dictionary<Material, HashSet<Decal>> dictionary = _deferredDecals[decal.RenderOrder];
		if (!dictionary.ContainsKey(decal.CurrentMaterial))
		{
			dictionary.Add(decal.CurrentMaterial, new HashSet<Decal> { decal });
		}
		else
		{
			dictionary[decal.CurrentMaterial].Add(decal);
		}
	}
	...
	protected void AddUnlit(Decal decal)
	{
		...
		Dictionary<Material, HashSet<Decal>> dictionary = _unlitDecals[decal.RenderOrder];
		if (!dictionary.ContainsKey(decal.CurrentMaterial))
		{
			dictionary.Add(decal.CurrentMaterial, new HashSet<Decal> { decal });
		}
