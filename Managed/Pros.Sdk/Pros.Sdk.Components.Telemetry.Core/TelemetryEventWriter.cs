...
public class TelemetryEventWriter
{
	...
	private async Task FlushLoop()
	{
		...
		while (!_cancellationSource.IsCancellationRequested)
		{
			...
			else if (_nextFlush - DateTime.UtcNow < TimeSpan.Zero)
			{
				...
				{
					_logger?.Log(HydraLogType.Error, this.GetLogCatErr(), "Telemetry flush loop error: {0}", err.GetErrorMessage());
					goto IL_0192;
				}
				continue;
			}
	...
	public async Task Shutdown()
	{
		...
		{
			_logger?.Log(HydraLogType.Error, this.GetLogCatErr(), "{0}", ex.Message);
		}
	}
}
