...
public class ResourceManager : IDisposable
{
	...
	private ListWithEvents<IUpdateReceiver> m_UpdateReceivers = new ListWithEvents<IUpdateReceiver>();
	...
	private DelegateList<float> m_UpdateCallbacks = DelegateList<float>.CreateWithGlobalCache();
	...
	private AsyncOperationHandle ProvideResource(IResourceLocation location, Type desiredType = null, bool releaseDependenciesOnFailure = true)
	{
		...
		LocationCacheKey locationCacheKey = new LocationCacheKey(location, desiredType);
		if (m_AssetOperationCache.TryGetValue(locationCacheKey, out var value))
		{
		...
		value = CreateOperation<IAsyncOperation>(value2, value2.GetHashCode(), locationCacheKey, m_ReleaseOpCached);
