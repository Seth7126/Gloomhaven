...
internal class InputActionState : IInputStateChangeMonitor, ICloneable, IDisposable
{
	...
	public struct UnmanagedMemory : IDisposable
	{
		...
		public unsafe void Allocate(int mapCount, int actionCount, int bindingCount, int controlCount, int interactionCount, int compositeCount)
		{
			...
			int num = sizeInBytes;
			byte* ptr = (byte*)UnsafeUtility.Malloc(num, 4, Allocator.Persistent);
	...
	public unsafe void RestoreActionStates(UnmanagedMemory oldState)
	{
		for (int i = 0; i < memory.bindingCount; i++)
		{
			...
			if (actionIndex != -1 && oldState.actionStates[actionIndex].phase != InputActionPhase.Disabled)
			{
	...
	private unsafe void OnBeforeInitialUpdate()
	{
		...
		for (int i = 0; i < totalBindingCount; i++)
		{
			BindingState* ptr = bindingStates + i;
			...
			for (int j = 0; j < controlCount; j++)
			{
				...
				if (!controls[num].CheckStateIsAtDefault())
				{
					...
					if (isComposite)
					{
						bindingIndex = controlIndexToBindingIndex[num];
					}
					ProcessControlStateChange(mapIndex, num, bindingIndex, currentTime, default(InputEventPtr));
					if (isComposite)
					{
						break;
					}
	...
	private unsafe void ProcessControlStateChange(int mapIndex, int controlIndex, int bindingIndex, double time, InputEventPtr eventPtr)
	{
		using (InputActionRebindingExtensions.DeferBindingResolution())
		{
			...
			try
			{
				...
				if (!flag2)
				{
					float num = ComputeMagnitude(ref trigger);
					TriggerState* ptr3 = actionStates + actionIndex;
					float num2 = ((controls[trigger.controlIndex] is ButtonControl buttonControl) ? buttonControl.pressPointOrDefault : ButtonControl.s_GlobalDefaultButtonPressPoint);
					if (!ptr3->isPressed && num >= num2)
					{
						ptr3->pressedInUpdate = InputUpdate.s_UpdateStepCount;
						ptr3->isPressed = true;
					}
					else if (ptr3->isPressed)
					{
						float num3 = num2 * ButtonControl.s_GlobalDefaultButtonReleaseThreshold;
						if (num <= num3)
						{
							ptr3->releasedInUpdate = InputUpdate.s_UpdateStepCount;
							ptr3->isPressed = false;
						}
					}
				}
	...
	private unsafe bool IsConflictingInput(ref TriggerState trigger, int actionIndex)
	{
		...
		if (trigger.magnitude < ptr->magnitude)
		{
			...
			for (int i = 0; i < bindingCount; i++)
			{
				...
				if (ptr2->isComposite)
				{
					...
					float num8 = memory.compositeMagnitudes[compositeOrCompositeBindingIndex4];
					if (num8 > 0f)
					{
						num6++;
					}
			...
			if (num4 != -1)
			{
				...
				if (ptr->bindingIndex != num5)
				{
					if (ptr->interactionIndex != -1)
					{
						ResetInteractionState(trigger.mapIndex, ptr->bindingIndex, ptr->interactionIndex);
					}
		...
		if (Mathf.Approximately(trigger.magnitude, ptr->magnitude))
		{
			...
			if (trigger.magnitude > 0f && num != ptr->controlIndex && controls[num] != controls[ptr->controlIndex])
			{
				ptr->hasMultipleConcurrentActuations = true;
			}
			return true;
	...
	private unsafe void ProcessDefaultInteraction(ref TriggerState trigger, int actionIndex)
	{
		...
		case InputActionPhase.Waiting:
			...
			{
				float num3 = ComputeMagnitude(ref trigger);
				if (num3 > 0f)
				{
					ChangePhaseOfAction(InputActionPhase.Started, ref trigger);
				}
				float num4 = ((controls[trigger.controlIndex] is ButtonControl buttonControl2) ? buttonControl2.pressPointOrDefault : ButtonControl.s_GlobalDefaultButtonPressPoint);
				if (num3 >= num4)
				{
		...
		case InputActionPhase.Started:
			if (ptr->isButton)
			{
				float num5 = ComputeMagnitude(ref trigger);
				float num6 = ((controls[trigger.controlIndex] is ButtonControl buttonControl3) ? buttonControl3.pressPointOrDefault : ButtonControl.s_GlobalDefaultButtonPressPoint);
				if (num5 >= num6)
				{
					ChangePhaseOfAction(InputActionPhase.Performed, ref trigger, InputActionPhase.Performed);
				}
				else if (Mathf.Approximately(num5, 0f))
				{
		...
		case InputActionPhase.Performed:
			...
			{
				float num = ComputeMagnitude(ref trigger);
				float num2 = ((controls[trigger.controlIndex] is ButtonControl buttonControl) ? buttonControl.pressPointOrDefault : ButtonControl.s_GlobalDefaultButtonPressPoint) * ButtonControl.s_GlobalDefaultButtonReleaseThreshold;
				if (num <= num2)
				{
					ChangePhaseOfAction(InputActionPhase.Canceled, ref trigger);
				}
	...
	private unsafe void ChangePhaseOfActionInternal(int actionIndex, TriggerState* actionState, InputActionPhase newPhase, ref TriggerState trigger)
	{
		...
		{
			triggerState.magnitude = (trigger.haveMagnitude ? trigger.magnitude : ComputeMagnitude(trigger.bindingIndex, trigger.controlIndex));
		}
		else
		{
	...
	internal unsafe BindingState GetBindingState(int bindingIndex)
	{
	...
	internal unsafe TValue ReadValue<TValue>(int bindingIndex, int controlIndex, bool ignoreComposites = false) where TValue : struct
	{
		...
		if (processorCount > 0)
		{
			...
			for (int i = 0; i < processorCount; i++)
			{
				...
				{
					output = inputProcessor.Process(output, inputControl);
				}
			}
		}
		return output;
	...
	public unsafe float EvaluateCompositePartMagnitude(int bindingIndex, int partNumber)
	{
		...
		for (int i = num; i < totalBindingCount && bindingStates[i].isPartOfComposite; i++)
		{
			if (bindingStates[i].partIndex == partNumber)
			{
				...
				for (int j = 0; j < controlCount; j++)
				{
					num2 = Mathf.Max(controls[controlStartIndex + j].EvaluateMagnitude(), num2);
				}
			}
		}
		return num2;
	...
	internal unsafe bool ReadCompositePartValue(int bindingIndex, int partNumber, void* buffer, int bufferSize)
	{
		...
		for (int i = num; i < totalBindingCount && bindingStates[i].isPartOfComposite; i++)
		{
			...
			for (int j = 0; j < controlCount; j++)
			{
				...
				float num4 = controls[num3].EvaluateMagnitude();
				if (!(num4 < num2))
				{
	...
	internal static void OnDeviceChange(InputDevice device, InputDeviceChange change)
	{
		for (int i = 0; i < s_GlobalState.globalList.length; i++)
		{
			...
			for (int k = 0; k < inputActionState.totalMapCount && !inputActionState.maps[k].LazyResolveBindings(); k++)
			{
			}
		}
	}
