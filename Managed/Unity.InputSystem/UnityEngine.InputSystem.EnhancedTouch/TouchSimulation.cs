...
[ExecuteInEditMode]
[HelpURL("https://docs.unity3d.com/Packages/com.unity.inputsystem@1.3/manual/Touch.html#touch-simulation")]
public class TouchSimulation : MonoBehaviour, IInputStateChangeMonitor
{
	...
	protected void AddPointer(Pointer pointer)
	{
		...
		{
			int count = m_NumPointers;
			ArrayHelpers.AppendWithCapacity(ref m_Pointers, ref m_NumPointers, pointer);
			ArrayHelpers.AppendWithCapacity(ref m_CurrentPositions, ref count, default(Vector2));
	...
	protected void RemovePointer(Pointer pointer)
	{
		...
		m_CurrentPositions.EraseAtWithCapacity(ref count, num);
		if (pointer.added)
		{
	...
	private unsafe void OnEvent(InputEventPtr eventPtr, InputDevice device)
	{
		...
		Vector2Control position = m_Pointers[num].position;
		...
		for (int i = 0; i < m_Touches.Length; i++)
		{
			...
			void* statePtrFromStateEventUnchecked2 = buttonControl.GetStatePtrFromStateEventUnchecked(eventPtr, type);
			...
			else if (buttonControl.ReadValueFromState(statePtrFromStateEventUnchecked2) < ButtonControl.s_GlobalDefaultButtonPressPoint * ButtonControl.s_GlobalDefaultButtonReleaseThreshold)
			{
		...
		foreach (InputControl item in eventPtr.EnumerateControls(InputControlExtensions.Enumerate.IgnoreControlsInDefaultState, device))
		{
			...
			void* statePtrFromStateEventUnchecked3 = item.GetStatePtrFromStateEventUnchecked(eventPtr, type);
			float output = 0f;
			item.ReadValueFromStateIntoBuffer(statePtrFromStateEventUnchecked3, UnsafeUtility.AddressOf(ref output), 4);
