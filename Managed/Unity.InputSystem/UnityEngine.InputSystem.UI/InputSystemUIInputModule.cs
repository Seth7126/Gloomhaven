...
[HelpURL("https://docs.unity3d.com/Packages/com.unity.inputsystem@1.3/manual/UISupport.html#setting-up-ui-input")]
public class InputSystemUIInputModule : BaseInputModule
{
	...
	{
		GameObject currentPointerTarget = (((eventData.pointerType == UIPointerType.Touch && !pointer.leftButton.isPressed && !pointer.leftButton.wasReleasedThisFrame) || (eventData.pointerType == UIPointerType.MouseOrPen && Cursor.lockState == CursorLockMode.Locked)) ? null : eventData.pointerCurrentRaycast.gameObject);
	...
	private int GetPointerStateIndexFor(InputControl control, bool createIfNotExists = true)
	{
		...
		if ((m_PointerBehavior == UIPointerBehavior.SingleUnifiedPointer && uIPointerType != UIPointerType.None) || (m_PointerBehavior == UIPointerBehavior.SingleMouseOrPenButMultiTouchAndTrack && uIPointerType == UIPointerType.MouseOrPen))
		{
			...
			{
				m_CurrentPointerIndex = AllocatePointer(num2, num3, uIPointerType, control, device, (num3 != 0) ? parent : null);
			}
			else
			{
	...
	private bool IgnoreNextClick(ref InputAction.CallbackContext context)
	{
		...
		if (context.canceled && !InputRuntime.s_Instance.isPlayerFocused)
		{
	...
	private InputActionReference UpdateReferenceForNewAsset(InputActionReference actionReference)
	{
		...
		InputActionReference inputActionReference = InputActionReference.Create(inputAction2);
		if (flag)
		{
			EnableInputAction(inputActionReference);
		}
