...
public class ExtendedPointerEventData : PointerEventData
{
	...
	internal void ReadDeviceState()
	{
		if (control.parent is Pen pen)
		{
			...
			base.pressure = pen.pressure.EvaluateMagnitude();
			base.azimuthAngle = (pen.tilt.ReadValue().x + 1f) * MathF.PI / 2f;
			base.altitudeAngle = (pen.tilt.ReadValue().y + 1f) * MathF.PI / 2f;
			base.twist = pen.twist.ReadValue() * MathF.PI * 2f;
		}
		else if (control.parent is TouchControl touchControl)
		{
			uiToolkitPointerId = GetTouchPointerId(touchControl);
			base.pressure = touchControl.pressure.EvaluateMagnitude();
			base.radius = touchControl.radius.ReadValue();
		}
		else if (control.parent is Touchscreen touchscreen)
		{
			uiToolkitPointerId = GetTouchPointerId(touchscreen.primaryTouch);
			base.pressure = touchscreen.pressure.EvaluateMagnitude();
			base.radius = touchscreen.radius.ReadValue();
		}
		else
		{
