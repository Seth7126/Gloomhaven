...
public struct InputUser : IEquatable<InputUser>
{
	...
	{
		int num = (asLostDevice ? s_GlobalState.allLostDevices.IndexOfReference(device, s_GlobalState.allLostDeviceCount) : s_GlobalState.allPairedDevices.IndexOfReference(device, s_GlobalState.allPairedDeviceCount));
		if (num == -1)
		{
			return;
		}
